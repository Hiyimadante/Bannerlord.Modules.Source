...
internal class ShipBurningSystem : ScriptComponentBehavior
{
	...
	public ShipBurningSystem()
	{
		...
		_railingNodes = new List<BurningNode>();
		_shipDeckNodes = new List<BurningNode>();
		_deckUpgradeNodes = new List<BurningNode>();
		_mastNodes = new List<BurningNode>();
		_soundNodes = new List<BurningSoundNode>();
		_burningLights = new List<Light>();
		_temporaryBurningNodes = new List<BurningNode>();
		_startFire = new SimpleButton();
		_stopFire = new SimpleButton();
		_spreadRate = 1f;
		_fireStartRandomCount = 2;
		_minFireProgressLight = 0.5f;
		_maxFireProgressLight = 1f;
		_maxLightIntensity = 5000f;
