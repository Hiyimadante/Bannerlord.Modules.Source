...
public class AgentNavalComponent : AgentComponent
{
	...
	private float _lastJumpOffTime;
	...
	public override void OnTick(float dt)
	{
		if (dt > 0f)
		{
			...
			{
				_lastJumpOffTime = ((base.Agent.IsOnLand() || base.Agent.IsInWater()) ? (-1f) : (Mission.Current.CurrentTime - 1.9f));
			}
			if (!BlockCheckingOffShipConsideration && base.Agent.IsAIControlled && _lastOffShipCheckTime + 5f <= base.Agent.Mission.CurrentTime && _navalAgentsLogic.IsDeploymentFinished)
			{
	...
	private void CheckAgentOffShip()
	{
		...
		{
			if (FormationShip != null && (FormationShip.Physics.NavalSinkingState != NavalDLC.Missions.NavalPhysics.NavalPhysics.SinkingState.Floating || FormationShip.BeingAbandoned))
			{
