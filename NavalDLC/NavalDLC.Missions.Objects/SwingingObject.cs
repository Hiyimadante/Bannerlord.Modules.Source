...
public class SwingingObject : MissionObject
{
	...
	internal SwingingObject()
	{
		...
		_damping = 5f;
		_centerOfMassHeight = -0.8f;
		_mass = 1f;
		_momentOfInertia = 0.5f;
		_resetSimulation = new SimpleButton();
		_testCollision = new SimpleButton();
		_accumulatedAcceleration = Vec3.Zero;
		_swingingEntity = WeakGameEntity.Invalid;
		_parentPrevVelocity = Vec3.Zero;
		_frameWrtDynamicRoot = MatrixFrame.Identity;
