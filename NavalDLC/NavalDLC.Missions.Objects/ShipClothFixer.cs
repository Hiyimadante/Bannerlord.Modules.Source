...
internal class ShipClothFixer : ScriptComponentBehavior
{
	...
	private ShipClothFixer()
	{
		...
		_shipCloths = new List<ClothData>();
		_prevPrevShipFrame = MatrixFrame.Identity;
		_prevShipFrame = MatrixFrame.Identity;
	...
	private void FetchClothComponents()
	{
		...
		foreach (WeakGameEntity item3 in list)
		{
			...
			for (int i = 0; i < componentCount; i++)
			{
				...
				GameEntityComponent componentAtIndex = ((WeakGameEntity)(ref current)).GetComponentAtIndex(i, (ComponentType)3);
				item.ClothComponent = (ClothSimulatorComponent)(object)((componentAtIndex is ClothSimulatorComponent) ? componentAtIndex : null);
			...
			if ((NativeObject)(object)((WeakGameEntity)(ref current)).Skeleton != (NativeObject)null)
			{
				...
				for (int j = 0; j < componentCount2; j++)
				{
					...
					GameEntityComponent componentAtIndex2 = ((WeakGameEntity)(ref current)).Skeleton.GetComponentAtIndex((ComponentType)3, j);
					item2.ClothComponent = (ClothSimulatorComponent)(object)((componentAtIndex2 is ClothSimulatorComponent) ? componentAtIndex2 : null);
					MatrixFrame globalFrame2 = ((WeakGameEntity)(ref current)).GetGlobalFrame();
