...
public class SailVisual : ScriptComponentBehavior
{
	...
	public bool FoldAnimationEnabled { get; set; }
	...
	internal SailVisual()
	{
		...
		_knobConnectionPoints = new List<KnobConnectionPoint>();
		_foldSailDuration = 3f;
		_foldedSailTransitionDuration = 0.5f;
		_foldFreeBoneResetDuration = 1.2f;
		_unfoldSailDuration = 4f;
		_foldSailStepMultiplier = 2f;
		_lateenRollChangeDegreeLimit = 20f;
		_lateenRollChangeAnimationDuration = 3f;
		_lateenRollChangeAnimationStepMultiplier = 1f;
		_lateenRollChangeYardShiftStart = 3f;
		_lateenRollChangeYardShiftDuration = 4f;
		_lateenRollChangeYardShiftAcceleration = 8f;
		_lateenRollDegrees = 45f;
		_ropeConnectionMaxDistance = 7f;
		_placeKnobButton = new SimpleButton();
		_placeKnobColor = Color.White;
		_startFireButton = new SimpleButton();
		_placeClothFragments = new SimpleButton();
		_burningAnimationDuration = 20f;
		_editorOnlyShipHealth = 1f;
		_topFlagRopePosition = 0.8f;
		_captureTheFlagBottomPosition = 0.25f;
		_startCaptureTheFlagAnimation = new SimpleButton();
		_freeBones = new List<FreeBoneRecord>();
		_simpleRopes = new List<SimpleRopeRecord>();
		_mastRopes = new List<SimpleRopeRecord>();
		_mastClipDistanceFromOrigin = 100f;
		_currentFrameGlobalWind = Vec3.Zero;
		_previousYawEntityFrame = MatrixFrame.Identity;
		_previousSailYardFrame = MatrixFrame.Identity;
		_lodCheckFirstFrame = true;
		_topLateenSails = new List<WeakGameEntity>();
		_topLateenFoldedSails = new List<WeakGameEntity>();
		_ballistaVisibilityRopes = new List<WeakGameEntity>();
		_currentSailLevelUsed = -1;
		_sailEntityAlpha = 1f;
		_lastMorphAnimKeySet = -1f;
		_remainingFramesForAnimation = 3;
		SailEnabled = true;
		FoldAnimationEnabled = true;
	...
	private void SetSailMaterialWrtLevel(Mesh mesh, int sailLevel, bool isEditorScene)
	{
		...
		{
			val = Material.GetFromResource(ShipVisual.CustomSailPatternId);
		}
		if ((NativeObject)(object)val == (NativeObject)null)
		{
			...
			val = Material.GetFromResource(text);
		}
		if (mesh.HasTag("faction_color"))
		{
			if ((NativeObject)(object)val != (NativeObject)null)
			{
				mesh.SetMaterial(val);
			}
			if (ShipVisual != null)
			{
