...
internal class RopeSegment : ScriptComponentBehavior
{
	...
	private RopeSegment()
	{
		...
		_looseAmount = 0.1f;
		_defaultRopeLength = 25.9f;
		_swingMultiplier = 1f;
		_bridgeCurveLinearAccessCache = new KeyValuePair<float, Vec3>[8];
		_firstTick = true;
		_previousPosition = Vec3.Zero;
		_previousVelocity = Vec3.Zero;
		_prevParentFrame = MatrixFrame.Identity;
		_tickRemainingForPhysics = 30;
		_currentFrameSwingFrame = MatrixFrame.Identity;
		_previousChangeDueToShip = Vec3.Zero;
		_ropeSegmentCosmetics = new List<RopeSegmentCosmetics>();
		_ropeSegmentCosmeticsDxCached = new List<float>();
