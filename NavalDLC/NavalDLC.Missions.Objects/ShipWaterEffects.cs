...
public class ShipWaterEffects : ScriptComponentBehavior
{
	...
	private class ParticleData
	{
		...
		public ParticleData()
		{
			...
			LocalFrame = MatrixFrame.Identity;
			SurfaceNormal = Vec3.Zero;
			WasAboveWater = true;
			SplashVelocity = Vec3.Zero;
			SplashPosition = Vec3.Zero;
			base._002Ector();
		}
	}
	...
	public ShipWaterEffects()
	{
		...
		_waterSimulationBoundingBox = Vec3.One;
		_resetWaterSimulationBoundingBox = new SimpleButton();
		_reRenderDepthTexture = new SimpleButton();
		_resetInHullWater = new SimpleButton();
		_hullWaterResScale = ResolutionScale.half;
		_hullWaterSplashWaterMultiplier = 1.75f;
		_hullWaterSplashPointInitialOffset = 0.5f;
		_hullWaterSplashPointSpeedMultiplier = 1f;
		_movementParticleHeightOffset = 0.34f;
		_splashParticleHeightOffset = 0.4f;
		_movementParticleSurfaceDistanceOffset = 0.7f;
		_splashParticleSurfaceDistanceOffset = 0.7f;
		_movementParticleSideSpeedVector = 0.5f;
		_waterVisualRecord = UIntPtr.Zero;
		_movementParticles = new List<ParticleData>();
		_splashParticles = new List<ParticleData>();
		_splashRandom = new MBFastRandom();
		_wetnessDecals = new List<WetnessDecalData>();
		_previousShipFrame = MatrixFrame.Identity;
		_smallSplashParticleIndex = -1;
		_mediumSplashParticleIndex = -1;
		_largeSplashParticleIndex = -1;
		_splashFoamDecals = new SplashFoamDecal[50];
		_lastDecalLeftSpawnPosition = Vec3.Zero;
		_lastDecalRightSpawnPosition = Vec3.Zero;
		_nextDecalLeftSpawnMetersSq = 49f;
		_nextDecalRightSpawnMetersSq = 49f;
		_previousShipFrameForDecalSpawn = Vec3.Zero;
		_leftDecalParticleIndex = -1;
		_rightDecalParticleIndex = -1;
	...
	private void CheckAndSpawnSplashes(float dt)
	{
		...
		gameEntity = ((ScriptComponentBehavior)this).GameEntity;
		WeakGameEntity root = ((WeakGameEntity)(ref gameEntity)).Root;
		((WeakGameEntity)(ref root)).GetGlobalFrame();
