...
public class NavalAgentStatCalculateModel : AgentStatCalculateModel
{
	...
	public override void InitializeMissionEquipmentAfterDeploymentFinished(Agent agent)
	{
		...
		if (Mission.Current.IsNavalBattle && agent.IsHuman)
		{
			...
			if (val != null)
			{
				...
				for (int i = 0; i < 5; i++)
				{
					...
					{
						((ExplainedNumber)(ref val5))._002Ector(0f, false, (TextObject)null);
						if (currentUsageItem.RelevantSkill == DefaultSkills.Throwing && val2 != null && val2.GetPerkValue(NavalPerks.Boatswain.WellStocked))
						{
							((ExplainedNumber)(ref val5)).Add(NavalPerks.Boatswain.WellStocked.SecondaryBonus, (TextObject)null, (TextObject)null);
						}
						int num = MathF.Round(((ExplainedNumber)(ref val5)).ResultNumber);
						((ExplainedNumber)(ref val6))._002Ector((float)(((MissionWeapon)(ref val4)).Amount + num), false, (TextObject)null);
						...
						{
							((ExplainedNumber)(ref val6)).AddFactor(NavalPerks.Boatswain.WellStocked.PrimaryBonus, (TextObject)null);
						}
						if (val2 != null && val2.GetPerkValue(NavalPerks.Boatswain.ShipwrightsInsight))
						{
							((ExplainedNumber)(ref val6)).AddFactor(NavalPerks.Boatswain.ShipwrightsInsight.SecondaryBonus, (TextObject)null);
						}
						int num2 = MathF.Round(((ExplainedNumber)(ref val6)).ResultNumber);
						if (num2 != ((MissionWeapon)(ref val4)).Amount)
						{
							equipment.SetAmountOfSlot(val3, (short)num2, true);
						}
					}
				}
			}
		...
		foreach (MissionShip item in (List<MissionShip>)(object)missionBehavior.AllShips)
		{
			if (!item.GetIsAgentOnShip(agent, bypassSteppedShipCheck: true) || Mission.Current.IsNavalRaidBattle)
			{
				continue;
			}
			...
			for (EquipmentIndex val7 = (EquipmentIndex)0; (int)val7 < 4; val7 = (EquipmentIndex)(val7 + 1))
			{
				MissionWeapon val8 = agent.Equipment[val7];
				if (((MissionWeapon)(ref val8)).IsEmpty)
				{
					continue;
				}
				val8 = agent.Equipment[val7];
				WeaponComponentData weaponComponentDataForUsage = ((MissionWeapon)(ref val8)).GetWeaponComponentDataForUsage(0);
				if (weaponComponentDataForUsage.IsShield)
				{
					if (flag)
					{
						MissionEquipment equipment2 = agent.Equipment;
						EquipmentIndex val9 = val7;
						val8 = agent.Equipment[val7];
						equipment2.SetHitPointsOfSlot(val9, (short)((float)((MissionWeapon)(ref val8)).ModifiedMaxHitPoints * (1f + item.ShipOrigin.CrewShieldHitPointsFactor)), true);
						flag = false;
					}
				}
				else
				{
					...
					if (weaponComponentDataForUsage.IsRangedWeapon)
					{
						if (flag3)
						{
							...
							EquipmentIndex val10 = val7;
							val8 = agent.Equipment[val7];
							equipment3.SetAmountOfSlot(val10, (short)(((MissionWeapon)(ref val8)).ModifiedMaxAmount * (1 + item.ShipOrigin.AdditionalThrowingWeaponStack)), true);
							EquipmentIndex val11 = val7;
							val8 = agent.Equipment[val7];
							agent.SetWeaponAmountInSlot(val11, ((MissionWeapon)(ref val8)).Amount, true);
							flag3 = false;
						}
					}
					else if (weaponComponentDataForUsage.IsAmmo && flag2)
					{
						...
						EquipmentIndex val12 = val7;
						val8 = agent.Equipment[val7];
						equipment4.SetAmountOfSlot(val12, (short)(((MissionWeapon)(ref val8)).ModifiedMaxAmount * (1 + item.ShipOrigin.AdditionalArcherQuivers)), true);
						EquipmentIndex val13 = val7;
						val8 = agent.Equipment[val7];
						agent.SetWeaponAmountInSlot(val13, ((MissionWeapon)(ref val8)).Amount, true);
						flag2 = false;
					}
				}
			}
