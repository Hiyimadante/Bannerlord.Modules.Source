...
public class NavalDLCShipDeploymentModel : ShipDeploymentModel
{
	...
	public override void GetMapEventPartiesOfPlayerTeams(MBReadOnlyList<MapEventParty> playerSideMapEventParties, bool isPlayerSergeant, out MapEventParty playerMapEventParty, out MBList<MapEventParty> playerTeamMapEventParties, out MBList<MapEventParty> playerAllyTeamMapEventParties)
	{
		...
		playerMapEventParty = ((IEnumerable<MapEventParty>)playerSideMapEventParties).FirstOrDefault((MapEventParty mep) => !mep.IsNpcParty);
		_ = mainParty.Army;
		...
		bool flag2 = MissionCombatantsLogic.SupportsAllyTeamOnPlayerSide((IEnumerable<IBattleCombatant>)((IEnumerable<MapEventParty>)playerSideMapEventParties).Select((MapEventParty mapEventParty) => mapEventParty.Party), (IBattleCombatant)(object)playerMapEventParty.Party, isPlayerSergeant, flag, ref val);
		foreach (MapEventParty item in (List<MapEventParty>)(object)playerSideMapEventParties)
		{
			if (PartyBase.IsPartyUnderPlayerCommand(item.Party) || !flag2)
			{
	...
	public override void FillShipsOfTeamParties(MBReadOnlyList<MapEventParty> teamMapEventParties, NavalShipDeploymentLimit shipDeploymentLimit, MBList<IShipOrigin> teamShips)
	{
		...
		if (num > 0)
		{
			bool flag2 = CanShipsBeFilled(troopCount, 0.65f, (MBReadOnlyList<(Ship ship, MapEventParty party, bool fixedShip)>)(object)candidateShips, out var firstUnfilledIndex);
			bool flag3 = true;
			while (flag3 && !flag2)
			{
				flag3 = false;
	...
	public override void GetOrderedCaptainsForPlayerTeamShips(MBReadOnlyList<MapEventParty> playerTeamMapEventParties, MBReadOnlyList<IShipOrigin> playerTeamShips, out List<string> playerTeamCaptainsByPriority)
	{
		...
		{
			MapEventParty shipParty = ((IEnumerable<MapEventParty>)playerTeamMapEventParties).FirstOrDefault((MapEventParty mep) => ((IEnumerable<IShipOrigin>)mep.Ships).Contains(ship));
	...
	{
		int num = 0;
		if (teamShips != null && ((List<IShipOrigin>)(object)teamShips).Count > 0)
		{
			int num2 = MathF.Min(8, ((List<IShipOrigin>)(object)teamShips).Count);
			if (isPlayerTeam)
			{
				List<IShipOrigin> list = ((IEnumerable<IShipOrigin>)teamShips).OrderByDescending((IShipOrigin ship) => ship.TotalCrewCapacity).ToList();
				for (int num3 = 0; num3 < num2; num3++)
				{
					num += list[num3].TotalCrewCapacity;
				}
			}
			else
			{
				...
				{
					num += ((List<IShipOrigin>)(object)teamShips)[num4].TotalCrewCapacity;
				}
			}
		}
		return num;
