...
public class NavalDLCShipCostModel : ShipCostModel
{
		...
		foreach (KeyValuePair<string, ShipSlot> availableSlot in ship.ShipHull.AvailableSlots)
		{
			...
			if (ship.UnlockedUpgradePieces != null)
			{
				for (int i = 0; i < ((List<ShipUpgradePiece>)(object)ship.UnlockedUpgradePieces).Count; i++)
				{
					...
					{
						int shipUpgradePieceValueInternal = GetShipUpgradePieceValueInternal(ship, val, owner, buyer);
	...
	public override float GetShipRepairCost(Ship ship, PartyBase owner)
	{
		...
		((ExplainedNumber)(ref val))._002Ector(GetShipBaseValue(ship, applyAiDiscount, owner, owner) * num * 0.25f, false, (TextObject)null);
	...
	public override int GetShipUpgradePieceCost(Ship ship, ShipUpgradePiece piece, PartyBase owner)
	{
		...
		if (unlockedUpgradePieces != null && ((List<ShipUpgradePiece>)(object)unlockedUpgradePieces).Contains(piece))
		{
			return 0;
		}
		foreach (KeyValuePair<string, ShipSlot> availableSlot in ship.ShipHull.AvailableSlots)
	...
	private static int GetShipUpgradePieceValueInternal(Ship ship, ShipUpgradePiece piece, PartyBase owner, PartyBase buyer)
	{
		...
		else
		{
			...
			obj = ((mobileParty2 != null) ? mobileParty2.ActualClan : null);
		}
		if (obj != Clan.PlayerClan)
		{
			...
			else
			{
				...
				obj2 = ((mobileParty3 != null) ? mobileParty3.ActualClan : null);
			}
