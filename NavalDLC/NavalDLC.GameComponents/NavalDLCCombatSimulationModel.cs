...
public class NavalDLCCombatSimulationModel : CombatSimulationModel
{
	public override CampaignTime GetSimulationTickInterval(MapEvent mapEvent)
	{
		...
		return ((MBGameModel<CombatSimulationModel>)this).BaseModel.GetSimulationTickInterval(mapEvent);
	}
	...
	public override (int defenderRounds, int attackerRounds) GetSimulationTicksForBattleRound(MapEvent mapEvent)
	{
		...
		if (mapEvent.IsRaid)
		{
			...
			if (mobileParty != null && mobileParty.IsCurrentlyAtSea)
			{
				...
				num4 = Math.Max(MathF.Round(MathF.Min((float)num6 * 3f, (float)numRemainingSimulationTroops * 0.3f)), 1);
				num5 = Math.Max(MathF.Round(MathF.Min((float)numRemainingSimulationTroops * 3f, (float)num6 * 0.3f)), 1);
				if ((int)mapEvent.RetreatingSide != -1)
				{
					if ((int)mapEvent.RetreatingSide == 1)
					{
	...
	public override MBList<(Ship, MapEventParty)> GetSimulationShips(MapEvent mapEvent, MBList<MapEventParty> battleParties)
	{
		...
		MBList<(Ship, MapEventParty)> val = new MBList<(Ship, MapEventParty)>();
		bool flag = (int)mapEvent.SimulationContext == 12;
		if (mapEvent.IsNavalMapEvent | flag)
		{
			foreach (MapEventParty item in (List<MapEventParty>)(object)battleParties)
			{
				foreach (Ship item2 in (List<Ship>)(object)item.Ships)
				{
					if (!flag || item2.ShipHull.CanNavigateShallowWater)
					{
						((List<(Ship, MapEventParty)>)(object)val).Add((item2, item));
					}
				}
			}
		}
	...
	public override int GetParticipatingTroopCount(MapEventSide side)
	{
		...
		int participatingTroopCount = ((MBGameModel<CombatSimulationModel>)this).BaseModel.GetParticipatingTroopCount(side);
		if (MapEventHelper.IsNavalRaid(side.MapEvent) && (int)side.MissionSide == 1 && (int)side.MapEvent.SimulationContext == 12)
		{
	...
	private int GetShallowShipDeckCrewCapacity(MapEventSide side)
	{
		...
		foreach (MapEventParty item in (List<MapEventParty>)(object)side.Parties)
		{
			foreach (Ship item2 in (List<Ship>)(object)item.Ships)
			{
				if (item2.ShipHull.CanNavigateShallowWater)
				{
					num += item2.MainDeckCrewCapacity;
				}
			}
		}
		return num;
