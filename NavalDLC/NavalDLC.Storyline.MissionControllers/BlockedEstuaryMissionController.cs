...
public class BlockedEstuaryMissionController : MissionLogic
{
	...
	private void CollectShips()
	{
		...
		Ship playerShipOrigin = ((IEnumerable<Ship>)MobileParty.MainParty.Ships).FirstOrDefault((Ship x) => ((MBObjectBase)x.ShipHull).StringId == "ship_trade_cog_q3") ?? ((IEnumerable<Ship>)MobileParty.MainParty.Ships).First();
		Ship enemyBurningShip = ((IEnumerable<Ship>)_enemyParty.Ships).FirstOrDefault((Ship x) => ((MBObjectBase)x.ShipHull).StringId == "burning_cog_ship");
		...
		_playerBurningShipOrigin = (IShipOrigin)(object)((IEnumerable<Ship>)MobileParty.MainParty.Ships).FirstOrDefault((Ship x) => ((MBObjectBase)x.ShipHull).StringId == "burning_fishing_ship");
	...
	public override void OnBehaviorInitialize()
	{
		...
		_shipAgentSpawnLogic = ((MissionBehavior)this).Mission.GetMissionBehavior<DefaultNavalMissionAgentSpawnLogic>();
	...
	private bool CanSeeShip(Agent agent, MissionShip ship)
	{
		...
		Vec3 position = agent.Position;
		WeakGameEntity gameEntity = ((ScriptComponentBehavior)ship).GameEntity;
		if (((Vec3)(ref position)).Distance(((WeakGameEntity)(ref gameEntity)).GlobalPosition) < 200f || ((IEnumerable<EnemyShipTrigger>)_triggers).Any(delegate(EnemyShipTrigger x)
		{
			//IL_002a: Unknown result type (might be due to invalid IL or missing references)
			//IL_0030: Invalid comparison between Unknown and I4
			return x.Ship.ShipOrder.TargetShip != null && x.Ship.ShipSiegeWeapon != null && (int)x.Ship.ShipSiegeWeapon.State == 2;
		}))
		{
			return true;
		}
