...
public class NavalStorylineTravelCommentaryCampaignBehavior : CampaignBehaviorBase
{
	...
	private bool IsLowOnTroopsOrShips()
	{
		if (!(MobileParty.MainParty.PartySizeRatio < 0.5f))
		{
			...
			{
				return ((IEnumerable<ShipTemplateStack>)partyData.Template.ShipHulls).Sum(delegate(ShipTemplateStack x)
				{
					//IL_0000: Unknown result type (might be due to invalid IL or missing references)
					return x.MaxValue;
				}) > ((List<Ship>)(object)MobileParty.MainParty.Ships).Count;
			}
			return false;
		}
		return true;
	...
	public NavalStorylineTravelCommentaryCampaignBehavior()
	{
		...
		RansLaundryLocation = new CampaignVec2(new Vec2(321.2f, 697.4f), false);
		FinisterreLocation = new CampaignVec2(new Vec2(202.24f, 621.91f), false);
		RookBirdLocation = new CampaignVec2(new Vec2(205f, 230f), false);
		TrandEstuaryLocation = new CampaignVec2(new Vec2(196.84f, 528.48f), false);
		GulfOfCharasLocation = new CampaignVec2(new Vec2(217f, 372f), false);
		GarantorCastleLocation = new CampaignVec2(new Vec2(359.33f, 304.45f), false);
		MarzoporLocation = new CampaignVec2(new Vec2(543.53f, 641.23f), false);
		GalendLocation = new CampaignVec2(new Vec2(195f, 437f), false);
		AngrafjordLocation = new CampaignVec2(new Vec2(260f, 770f), false);
		ByalicLocation = new CampaignVec2(new Vec2(555f, 724.72f), false);
		_finisterCooldown = CampaignTime.Zero;
		_byalicCooldown = CampaignTime.Zero;
		_rookCooldown = CampaignTime.Zero;
		_ransLaundryCooldown = CampaignTime.Zero;
		_trandEstuaryCooldown = CampaignTime.Zero;
		_charasCooldown = CampaignTime.Zero;
		_garantorCastleCooldown = CampaignTime.Zero;
		_mazoporCooldown = CampaignTime.Zero;
		_galendCooldown = CampaignTime.Zero;
		_nextGoToPortEventTime = CampaignTime.Zero;
