...
public class NavalStorylineCampaignBehavior : CampaignBehaviorBase
{
	...
	private sealed class _003C_003Ec
	{
		...
		public static Func<LocationCharacter, bool> _003C_003E9__64_0;
		...
		internal bool _003Ctalk_to_gunnar_town_outside_on_condition_003Eb__64_0(LocationCharacter t)
		{
			if (t.Character != null && ((BasicCharacterObject)t.Character).IsHero)
			{
				return t.Character.HeroObject == NavalStorylineData.Gunnar;
			}
			return false;
		}
	}
	...
	private void OnGameLoadFinished()
	{
		...
		if (NavalStorylineData.GetStorylineStage() >= NavalStorylineData.NavalStorylineStage.Act3Quest5 && !NavalStorylineData.Gunnar.IsDead)
		{
			...
			Village val = ((IEnumerable<Village>)Village.All).FirstOrDefault((Village x) => ((MBObjectBase)((SettlementComponent)x).Settlement).StringId == "village_N1_2");
	...
	private void SpawnSister(Vec3 spawnPosition)
	{
		...
		Agent val = ((IEnumerable<Agent>)Mission.Current.Agents).FirstOrDefault((Agent x) => IsSister((IAgent)(object)x));
		if (val == null)
		{
	...
	private void AddGameMenus(CampaignGameStarter campaignGameStarter)
	{
		...
		campaignGameStarter.AddGameMenu("talk_to_gunnar_restricted", "{=sNybnI5O}Gunnar's contact snuck you into the town and lead you to him.", new OnInitDelegate(talk_to_gunnar_restricted_on_init), (MenuOverlayType)0, (MenuFlags)0, (object)null);
	...
	private bool talk_to_gunnar_town_outside_on_condition(MenuCallbackArgs args)
	{
		...
		LocationComplex current = LocationComplex.Current;
		if (storylineStage >= NavalStorylineData.NavalStorylineStage.Act1 && storylineStage != NavalStorylineData.NavalStorylineStage.Act3Quest5 && !NavalStorylineData.IsMainPartyAllowed() && current != null && LinQuick.AnyQ<LocationCharacter>((IEnumerable<LocationCharacter>)current.GetListOfCharacters(), (Func<LocationCharacter, bool>)((LocationCharacter t) => t.Character != null && ((BasicCharacterObject)t.Character).IsHero && t.Character.HeroObject == NavalStorylineData.Gunnar)) && currentSettlement != null && !currentSettlement.IsUnderSiege)
		{
	...
	public NavalStorylineCampaignBehavior()
	{
		...
		_lastCompletedStorylineStage = NavalStorylineData.NavalStorylineStage.None;
		_isFirstReturnToOstican = true;
		_sisterReturnTime = CampaignTime.Zero;
		_foodStage = 1;
