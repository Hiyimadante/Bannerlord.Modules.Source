...
public class NavalShipsLogic : MissionLogic, IVehicleHandler, IMissionBehavior
{
	...
	public void AddShipSiegeEngineMissile(Missile missile)
	{
		if (!_shipSiegeEngineMissileDictionary.ContainsKey(((MBMissile)missile).Index))
		{
			...
			AddShipSiegeEngineMissileEvent?.Invoke(missile);
		}
	}

	private void AddMissileHittingSail(Missile missile)
	{
		//IL_0020: Unknown result type (might be due to invalid IL or missing references)
		//IL_0025: Unknown result type (might be due to invalid IL or missing references)
		if (!_missileHittingSailDictionary.ContainsKey(((MBMissile)missile).Index))
		{
			Dictionary<int, float> missileHittingSailDictionary = _missileHittingSailDictionary;
			int index = ((MBMissile)missile).Index;
			Vec3 velocity = ((MBMissile)missile).GetVelocity();
			missileHittingSailDictionary.Add(index, ((Vec3)(ref velocity)).Length);
		}
	}
	...
	public override void AfterStart()
	{
		...
		SeaPathfindingEnabled = (((MissionBehavior)this).Mission.IsNavalBattle || ((MissionBehavior)this).Mission.IsNavalRaidBattle) && ((MissionBehavior)this).Mission.Scene.SetAbilityOfFacesWithId(1, false) > 0;
	}
	...
	public override void OnMissionTick(float dt)
	{
		...
		foreach (Missile item in (List<Missile>)(object)((MissionBehavior)this).Mission.MissilesList)
		{
			...
			foreach (MissionShip item2 in (List<MissionShip>)(object)AllShips)
			{
				Agent shooterAgent = item.ShooterAgent;
				missileWeapon = item.Weapon;
				missionSail = item2.CheckHitSails(shooterAgent, item, in missileOldPosition, in missilePosition, in missileWeapon);
				if (missionSail != null)
				{
					HandleSailsHit(item2, missionSail, item.ShooterAgent, item, item.Weapon);
					item.PassThroughEntity(missionSail.SailEntity);
				}
			}
		}
	}
	...
	public void OnShipCollision(MissionShip ship, WeakGameEntity targetEntity, BodyFlags bodyFlags, Vec3 averageContactPoint, Vec3 totalImpulseOnShip, bool isFirstImpact)
	{
		...
		ShipCollisionEvent?.Invoke(ship, targetEntity, bodyFlags, averageContactPoint, totalImpulseOnShip, isFirstImpact);
	}
	...
	{
		//IL_0010: Unknown result type (might be due to invalid IL or missing references)
		ShipRammingEvent?.Invoke(rammingShip, rammedShip, damagePercent, isFirstImpact, capsuleData, ramQuality);
	}

	public void OnShipsConnected(MissionShip ownerShip, MissionShip targetShip)
	{
		ShipsConnectedEvent?.Invoke(ownerShip, targetShip);
	}

	public void OnSuccessfulHookThrow(MissionShip hookingShip, MissionShip hookedShip)
	{
		ShipHookThrowEvent?.Invoke(hookingShip, hookedShip);
	}
	...
	public void OnShipHit(MissionShip ship, Agent attackerAgent, int damage, Vec3 impactPosition, Vec3 impactDirection, in MissionWeapon weapon, int affectorWeaponSlotOrMissileIndex)
	{
		...
		ShipHitEvent?.Invoke(ship, attackerAgent, damage, impactPosition, impactDirection, weapon, affectorWeaponSlotOrMissileIndex);
	}

	public void OnShipPreparedForAbandonment(MissionShip ship)
	{
		ShipPreparedForAbandonmentEvent?.Invoke(ship);
	}

	public void OnShipBurned(MissionShip ship)
	{
		ShipBurnedEvent?.Invoke(ship);
	}

	public void OnShipAttachmentLost(MissionShip hookingShip, MissionShip hookedShip)
	{
		ShipAttachmentLostEvent?.Invoke(hookingShip, hookedShip);
	}

	public void OnSailsDead(MissionShip ship)
	{
		SailsDeadEvent?.Invoke(ship);
	}

	public void OnShipLowHealth(MissionShip ship)
	{
		ShipLowHealthEvent?.Invoke(ship);
	}
	...
	{
		ShipAboutToBeRammedEvent?.Invoke(rammingShip, rammedShip, distance, speedInRamDirection);
	}

	public void OnCutLooseOrder(MissionShip ship)
	{
		CutLooseOrderEvent?.Invoke(ship);
	}

	public void OnBoardingOrder(MissionShip boardingShip, MissionShip boardedShip)
	{
		BoardingOrderEvent?.Invoke(boardingShip, boardedShip);
	}

	public void OnBridgeConnected(MissionShip sourceShip, MissionShip targetShip)
	{
		BridgeConnectedEvent?.Invoke(sourceShip, targetShip);
	}
	...
	public bool GetCollisionFreeShipFrame(in MatrixFrame shipFrame, in Vec2 shipDimensions, out MatrixFrame collisionFreeFrame, bool checkBoundaries = true, NavalBoundaryCheckType boundaryToCheck = NavalBoundaryCheckType.HardBoundary, Team team = null, float searchCellWidth = 10f, int searchCellDistance = 10, float clearanceMargin = 1f, int ignoreShipIndex = -1)
	{
		...
		bool flag = false;
		Oriented2DArea area = new Oriented2DArea(ref Vec2.Zero, ref val3, ref shipDimensions);
		do
		{
			...
			Vec2 position = asVec + ((Vec2)(ref val2)).RightVec() * searchCellWidth * (float)item.X + ((Oriented2DArea)(ref area)).GlobalForward * searchCellWidth * (float)item.Y;
		...
		if (flag)
		{
			...
			collisionFreeFrame.rotation.f = ((Vec2)(ref val3)).ToVec3(0f);
	...
	public bool IsShipFrameCollisionFree(in MatrixFrame frame, in Vec2 localDimensions, bool checkBoundaries = true, NavalBoundaryCheckType boundaryToCheck = NavalBoundaryCheckType.HardBoundary, Team team = null, float clearanceMargin = 1f, int ignoreShipIndex = -1)
	{
		...
		Vec3 val = frame.origin;
		Vec2 position = ((Vec3)(ref val)).AsVec2;
		val = frame.rotation.f;
		Vec2 asVec = ((Vec3)(ref val)).AsVec2;
		Vec2 val2 = ((Vec2)(ref asVec)).Normalized();
		Oriented2DArea area = new Oriented2DArea(ref position, ref val2, ref localDimensions);
		if (!checkBoundaries || IsPositionInsideBoundaries(in position, boundaryToCheck, team))
		{
	...
	public void TeleportShip(MissionShip ship, MatrixFrame targetFrame, bool checkFreeArea, bool anchorShip = false, bool snapToWater = true)
	{
		...
		ShipTeleportedEvent?.Invoke(ship, globalFrame, targetFrame);
	}
	...
	public MissionShip SpawnShip(Formation formation, in MatrixFrame spawnFrame, bool spawnAnchored = true, bool checkForFreeArea = true)
	{
		...
		if (((MatrixFrame)(ref shipFrame)).IsZero & flag)
		{
			shipFrame = deploymentPlan.GetDeploymentFrame(formation.Team);
		}
		if (((MatrixFrame)(ref shipFrame)).IsZero)
		{
			shipFrame = GetMeanFrameOfTeamShips(teamSide);
		}
		if (checkForFreeArea && GetCollisionFreeShipFrame(in shipFrame, shipAssignment.MissionShipObject.DeploymentArea, out var collisionFreeFrame, checkBoundaries: true, flag ? NavalBoundaryCheckType.DeploymentBoundary : NavalBoundaryCheckType.HardBoundary, formation.Team, 1f, 400))
		{
			shipFrame = collisionFreeFrame;
		}
		float waterLevelAtPosition = ((MissionBehavior)this).Mission.Scene.GetWaterLevelAtPosition(((Vec3)(ref shipFrame.origin)).AsVec2, true, true);
		...
		ShipSpawnedEvent?.Invoke(missionShip);
		return missionShip;
	}
	...
	public void TransferShipToTeam(MissionShip ship, Team targetTeam, Formation targetFormation = null, FormationClass searchRange = (FormationClass)8)
	{
		...
		bool flag = FindAssignmentOfShipOrigin(ship.ShipOrigin, out var shipAssignment);
		if (targetFormation == null)
		{
			targetFormation = FindFirstFormationWithoutShip(targetTeam, searchRange);
		}
		BeforeShipTransferredToTeamEvent?.Invoke(ship, targetTeam, targetFormation);
		Team team = ship.Team;
		...
		ShipTransferredToTeamEvent?.Invoke(ship, team, formation);
	}
	...
	public void OnShipCaptured(MissionShip ship, Formation targetFormation)
	{
		...
		//IL_0022: Unknown result type (might be due to invalid IL or missing references)
		Formation formation = ship.Formation;
		bool num = FindAssignmentOfShipOrigin(ship.ShipOrigin, out var shipAssignment);
		ShipAssignment shipAssignment2 = GetShipAssignment(targetFormation.Team.TeamSide, targetFormation.FormationIndex);
		...
		{
			item.ShipOrder.OnShipCaptured(ship, missionShip);
		}
		((TeamAINavalComponent)(object)((formation != null) ? formation.Team.TeamAI : null))?.TeamNavalQuerySystem.ForceExpireAll();
		((TeamAINavalComponent)(object)targetFormation.Team.TeamAI).TeamNavalQuerySystem.ForceExpireAll();
		ShipCapturedEvent?.Invoke(ship, missionShip, formation, targetFormation);
		if (formation != null)
		{
	...
	public void TransferShipToFormation(IShipOrigin shipOrigin, Formation fromFormation, Formation toFormation)
	{
		...
		MissionShip missionShip = shipAssignment.MissionShip;
		BeforeShipTransferredToFormationEvent?.Invoke(missionShip, toFormation);
		...
		missionShip.ResetFormationPositioning();
		ShipTransferredToFormationEvent?.Invoke(missionShip, fromFormation);
	}
	...
	private void RemoveShipAux(ShipAssignment shipAssignment)
	{
		...
		foreach (MissionShip item in (List<MissionShip>)(object)AllShips)
		{
			if (item != shipToRemove && item.HasController && item.Controller.IsAIControlled && item.AIController.TargetShip == shipToRemove)
			{
				item.AIController.ClearTarget();
			}
		}
	}
	...
	private void TeleportShipAux(MissionShip ship, ref MatrixFrame targetFrame, bool checkFreeArea, bool anchorShip = false, bool snapToWater = true)
	{
		...
		//IL_004b: Unknown result type (might be due to invalid IL or missing references)
		//IL_004c: Unknown result type (might be due to invalid IL or missing references)
		if (checkFreeArea && GetCollisionFreeShipFrame(in targetFrame, ship.MissionShipObject.DeploymentArea, out var collisionFreeFrame, checkBoundaries: true, ((int)((MissionBehavior)this).Mission.Mode == 6) ? NavalBoundaryCheckType.DeploymentBoundary : NavalBoundaryCheckType.HardBoundary, ship.Team, 1f, 400, 1f, ship.Index))
		{
	...
	internal void HandleSailsHit(MissionShip ship, MissionSail sail, Agent attackerAgent, Missile missile, MissionWeapon missileWeapon)
	{
		//IL_0030: Unknown result type (might be due to invalid IL or missing references)
		//IL_0035: Unknown result type (might be due to invalid IL or missing references)
		float num = ((MissionWeapon)(ref missileWeapon)).CurrentUsageItem.FireDamage;
		float missileVelocityLengthOnFirstSailHit = GetMissileVelocityLengthOnFirstSailHit(((MBMissile)missile).Index);
		...
		float inflictedDamage = MissionGameModels.Current.AgentApplyDamageModel.CalculateSailFireDamage(attackerAgent, ship.ShipOrigin, num, IsMissileFromShipSiegeEngine(((MBMissile)missile).Index));
