...
public class LobbyState : GameState
{
	...
	{
		ClientRefusedToJoinCustomServer?.Invoke(serverEntry);
	}
	...
	public void CreatePremadeGame()
	{
		...
		if ((int)val == 1)
		{
			...
			{
				if (NetworkMain.GameClient.PlayersInClan.FirstOrDefault(delegate(ClanPlayer clanPlayer)
				{
					//IL_0001: Unknown result type (might be due to invalid IL or missing references)
					//IL_000c: Unknown result type (might be due to invalid IL or missing references)
					return clanPlayer.PlayerId == partyPlayer.PlayerId;
				}) == null)
				{
					flag = false;
				}
	...
	private void MultiplayerPermissionWithPlayerChanged(PlayerId targetPlayerId, Permission permission, bool hasPermission)
	{
		...
		if (!hasPermission && NetworkMain.GameClient.PlayersInParty.FirstOrDefault(delegate(PartyPlayerInLobbyClient p)
		{
			//IL_0001: Unknown result type (might be due to invalid IL or missing references)
			//IL_0007: Unknown result type (might be due to invalid IL or missing references)
			return p.PlayerId == targetPlayerId;
		}) != null)
		{
			NetworkMain.GameClient.KickPlayerFromParty(NetworkMain.GameClient.PlayerID);
		}
