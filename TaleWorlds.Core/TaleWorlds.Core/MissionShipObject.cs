...
public class MissionShipObject : MBObjectBase
{
	...
	{
		bool flag = true;
		string text = DeserializeScalarAttribute<string>(node, "base_mission_ship", isRequiredAttribute: false, out var isAttributeValid);
		...
		string text2 = DeserializeScalarAttribute<string>(node, "prefab", flag, out var isAttributeValid2);
		...
		string text3 = DeserializeScalarAttribute<string>(node, "ship_physics_reference_id", flag, out var isAttributeValid3);
		...
		float num = DeserializeScalarAttribute<float>(node, "mass", flag, out var isAttributeValid4);
		...
		float num2 = DeserializeFloatAttribute(node, "floating_force_multiplier", isRequiredAttribute: false, out var isAttributeValid5, 0.01f, 5f);
		...
		float num3 = DeserializeFloatAttribute(node, "maximum_submerged_volume_ratio", isRequiredAttribute: false, out var isAttributeValid6, 0.01f, 5f);
		...
		float num4 = DeserializeFloatAttribute(node, "oars_tip_speed", flag, out var isAttributeValid7);
		...
		float num5 = DeserializeFloatAttribute(node, "oarsmen_force_multiplier", isRequiredAttribute: false, out var isAttributeValid8);
		...
		float num6 = DeserializeFloatAttribute(node, "oar_friction_multiplier", isRequiredAttribute: false, out var isAttributeValid9);
		...
		int oarCount = DeserializeScalarAttribute<int>(node, "oar_count", flag, out var isAttributeValid10);
		...
		float num7 = DeserializeScalarAttribute<float>(node, "rudder_blade_length", flag, out var isAttributeValid11);
		...
		float num8 = DeserializeScalarAttribute<float>(node, "rudder_blade_height", flag, out var isAttributeValid12);
		...
		float num9 = DeserializeScalarAttribute<float>(node, "rudder_deflection_coef", flag, out var isAttributeValid13);
		...
		float num10 = DeserializeScalarAttribute<float>(node, "rudder_rotation_max", flag, out var isAttributeValid14) * (System.MathF.PI / 180f);
		...
		float num11 = DeserializeScalarAttribute<float>(node, "rudder_rotation_rate", flag, out var isAttributeValid15) * (System.MathF.PI / 180f);
		...
		float num12 = DeserializeScalarAttribute<float>(node, "rudder_force_max", flag, out var isAttributeValid16);
		...
		float num13 = DeserializeFloatAttribute(node, "max_linear_speed", flag, out var isAttributeValid17, 1f, 100f, "m/s");
		...
		float num14 = DeserializeFloatAttribute(node, "max_linear_acceleration", flag, out var isAttributeValid18, 0.1f, 50f, "m/s^2");
		...
		float num15 = DeserializeFloatAttribute(node, "max_angular_speed", flag, out var isAttributeValid19, 1f, 180f, "deg") * (System.MathF.PI / 180f);
		...
		float num16 = DeserializeFloatAttribute(node, "max_angular_acceleration", flag, out var isAttributeValid20, 1f, 180f, "deg/s") * (System.MathF.PI / 180f);
		...
		float num17 = DeserializeFloatAttribute(node, "bow_angle_limit_from_centerline", flag, out var isAttributeValid21, 0f, 90f);
		...
		float num18 = DeserializeFloatAttribute(node, "landing_depth", flag, out var isAttributeValid22, 0f, 90f);
		...
		Vec2 deploymentArea = Deserialize2DDimensionElement(node, "deployment_area", flag, out var isElementValid);
		if (isElementValid)
		{
			if (!flag)
			{
		...
		MBList<ShipSail> mBList = DeserializeSailsElement(node, flag, out var isElementValid2);
		if (isElementValid2)
		{
		...
		Vec3 momentOfInertiaMultiplier = DeserializeVectorElement(node, "moment_of_inertia_multiplier", isRequiredElement: false, out var isElementValid3);
		if (isElementValid3)
		{
			if (!flag)
			{
		...
		LinearFrictionTerm linearFrictionMultiplier = DeserializeLinearDragTermElement(node, "linear_friction_multiplier", out var isElementValid4);
		if (isElementValid4)
		{
			if (!flag)
			{
		...
		Vec3 angularFrictionMultiplier = DeserializeAngularDragTermElement(node, "angular_friction_multiplier", out var isElementValid5);
		if (isElementValid5)
		{
			if (!flag)
			{
		...
		float num19 = DeserializeFloatAttribute(node, "lateral_drag_shift_max", isRequiredAttribute: false, out var isAttributeValid23, 0f, 100f, "m");
		...
		float num20 = DeserializeFloatAttribute(node, "lateral_drag_shift_critical_angle", isRequiredAttribute: false, out var isAttributeValid24, 0f, 90f, "deg") * (System.MathF.PI / 180f);
		...
		Vec3 rudderStockPosition = DeserializeVectorElement(node, "rudder_stock_position", isRequiredElement: false, out var isElementValid6);
		if (isElementValid6)
		{
			if (!flag)
			{
		...
		float num21 = DeserializeFloatAttribute(node, "maximum_submerged_volume_ratio", isRequiredAttribute: false, out var isAttributeValid25);
		...
		float num22 = DeserializeFloatAttribute(node, "partial_hit_points_ratio", flag, out var isAttributeValid26);
		...
		float num23 = DeserializeFloatAttribute(node, "torque_multiplier_of_lateral_buoyant_forces", isRequiredAttribute: false, out var isAttributeValid27);
		...
		Vec3 torqueMultiplierOfVerticalBuoyantForces = DeserializeVectorElement(node, "torque_multiplier_of_vertical_buoyant_forces", isRequiredElement: false, out var isElementValid7);
		if (isElementValid7)
		{
			if (!flag)
			{
	...
	private MBList<ShipSail> DeserializeSailsElement(XmlNode node, bool hasAttributeRequirement, out bool isElementValid)
	{
		...
		if (isElementValid)
		{
			for (int i = 0; i < xmlNode.ChildNodes.Count; i++)
			{
				...
				SailType type = DeserializeSailTypeAttribute(node2, "type", isRequiredAttribute: true, out var isAttributeValid);
