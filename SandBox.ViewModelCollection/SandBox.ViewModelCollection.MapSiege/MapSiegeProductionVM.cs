...
public class MapSiegeProductionVM : ViewModel
{
	...
	private void OnPossibleMachineSelection(MapSiegeProductionMachineVM machine)
	{
		...
		if (LatestSelectedPOI.Machine == null || LatestSelectedPOI.Machine.SiegeEngine != machine.Engine)
		{
			...
			{
				SiegeEngineConstructionProgress val = ((IEnumerable<SiegeEngineConstructionProgress>)siegeEventSide.SiegeEngines.ReservedSiegeEngines).FirstOrDefault((SiegeEngineConstructionProgress e) => e.SiegeEngine == machine.Engine);
				if (val == null)
				{
