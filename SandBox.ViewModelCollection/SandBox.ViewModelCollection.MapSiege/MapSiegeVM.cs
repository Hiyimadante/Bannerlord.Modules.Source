...
public class MapSiegeVM : ViewModel
{
	...
	public void OnSelectionFromScene(MatrixFrame frameOfEngine)
	{
		...
		{
			((IEnumerable<MapSiegePOIVM>)PointsOfInterest).Where(delegate(MapSiegePOIVM poi)
			{
				//IL_0007: Unknown result type (might be due to invalid IL or missing references)
				return ((MatrixFrame)(ref frameOfEngine)).NearlyEquals(poi.MapSceneLocationFrame, 1E-05f);
			})?.FirstOrDefault()?.ExecuteSelection();
		}
	}
