...
public class CaravanTransactionNotificationItemVM : SettlementNotificationItemBaseVM
{
	...
	public CaravanTransactionNotificationItemVM(Action<SettlementNotificationItemBaseVM> onRemove, MobileParty caravanParty, List<(EquipmentElement, int)> items, int createdTick)
	{
		...
		base._002Ector(onRemove, createdTick);
		_items = items;
		List<(EquipmentElement, int)> list = Extensions.DistinctBy<(EquipmentElement, int), EquipmentElement>((IEnumerable<(EquipmentElement, int)>)_items, (Func<(EquipmentElement, int), EquipmentElement>)delegate((EquipmentElement, int) i)
		{
