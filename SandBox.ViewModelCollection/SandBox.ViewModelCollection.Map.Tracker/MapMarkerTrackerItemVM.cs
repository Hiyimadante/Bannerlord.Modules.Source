...
public class MapMarkerTrackerItemVM : MapTrackerItemVM<MapMarker>
{
	...
	protected override IssueQuestFlags GetRelatedQuests()
	{
		...
		QuestBase val = ((IEnumerable<QuestBase>)Campaign.Current.QuestManager.Quests).FirstOrDefault((QuestBase q) => ((MBObjectBase)q).StringId == base.TrackedObject.QuestId);
		if (val != null)
		{
