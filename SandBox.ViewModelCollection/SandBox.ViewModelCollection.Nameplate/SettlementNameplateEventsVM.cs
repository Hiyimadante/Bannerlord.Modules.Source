...
public class SettlementNameplateEventsVM : ViewModel
{
	...
	private void RefreshQuestCounts()
	{
		...
		if (Campaign.Current.QuestManager.TrackedObjects.TryGetValue((ITrackableCampaignObject)(object)_settlement, ref list))
		{
			...
			{
				if (item.IsSpecialQuest && !((IEnumerable<QuestMarkerVM>)TrackQuests).Any(delegate(QuestMarkerVM x)
				{
					//IL_0001: Unknown result type (might be due to invalid IL or missing references)
					//IL_0008: Invalid comparison between Unknown and I4
					return (int)x.IssueQuestFlag == 16;
				}))
				{
				...
				else if (!((IEnumerable<QuestMarkerVM>)TrackQuests).Any(delegate(QuestMarkerVM x)
				{
					//IL_0001: Unknown result type (might be due to invalid IL or missing references)
					//IL_0007: Invalid comparison between Unknown and I4
					return (int)x.IssueQuestFlag == 8;
				}))
				{
