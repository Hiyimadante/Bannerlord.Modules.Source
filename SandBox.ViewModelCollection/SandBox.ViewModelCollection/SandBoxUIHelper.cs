...
public static class SandBoxUIHelper
{
	...
	public static bool CanAgentBeAlarmed(Agent agent)
	{
		...
		{
			return (agent.GetAgentFlags() & 0x10000) == 65536;
		}
		return false;
	}
	...
	static SandBoxUIHelper()
	{
		...
		_soldStr = new TextObject("{=YgyHVu8S}Sold{ITEMS}", (Dictionary<string, object>)null);
		_purchasedStr = new TextObject("{=qIeDZoSx}Purchased{ITEMS}", (Dictionary<string, object>)null);
		_itemTransactionStr = new TextObject("{=CqAhj27p} {ITEM_NAME} x{ITEM_NUMBER}", (Dictionary<string, object>)null);
		_lootStr = new TextObject("{=nvemmBZz}You earned {AMOUNT}% of the loot and prisoners", (Dictionary<string, object>)null);
	}
}
