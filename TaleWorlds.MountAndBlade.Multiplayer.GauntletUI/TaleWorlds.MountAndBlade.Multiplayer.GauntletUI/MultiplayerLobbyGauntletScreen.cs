...
public class MultiplayerLobbyGauntletScreen : ScreenBase, IGameStateListener, ILobbyStateHandler, IChatLogHandlerScreen
{
	...
	private void CreateView()
	{
		...
		if (NativeOptions.GetConfig((NativeOptionsType)69) < 2f)
		{
	...
	{
		bool flag = ((ScreenLayer)_lobbyLayer).Input.IsHotKeyReleased("Confirm");
		bool flag2 = ((ScreenLayer)_lobbyLayer).Input.IsHotKeyReleased("ToggleFriendsList");
		if (flag | flag2)
		{
	...
	private void SetHotKey(Key key)
	{
		...
		{
			MPLobbyVM lobbyDataSource = _lobbyDataSource;
			if (((lobbyDataSource != null) ? ((IEnumerable<GameKeyGroupVM>)((OptionsVM)lobbyDataSource.Options).GameKeyOptionGroups.GameKeyGroups).FirstOrDefault((GameKeyGroupVM g) => ((Collection<GameKeyOptionVM>)(object)g.GameKeys).Contains(gameKey)) : null) == null)
			{
		...
		else
		{
			...
			{
				if (((IEnumerable<AuxiliaryKeyGroupVM>)((OptionsVM)_lobbyDataSource.Options).GameKeyOptionGroups.AuxiliaryKeyGroups).FirstOrDefault((AuxiliaryKeyGroupVM g) => ((Collection<AuxiliaryKeyOptionVM>)(object)g.HotKeys).Contains(auxiliaryKey)) == null)
				{
		...
		if (options != null)
		{
			GameKeyOptionCategoryVM gameKeyOptionGroups = ((OptionsVM)options).GameKeyOptionGroups;
