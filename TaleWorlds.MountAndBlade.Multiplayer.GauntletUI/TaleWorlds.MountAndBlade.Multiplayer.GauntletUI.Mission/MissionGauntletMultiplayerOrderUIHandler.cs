...
public class MissionGauntletMultiplayerOrderUIHandler : GauntletOrderUIHandler
{
	...
	public void InitializeInADisgustingManner()
	{
		...
		if (base._isInitialized)
		{
			Debug.Print("InitializeInADisgustingManner called while already initialized!", 0, (DebugColor)12, 17592186044416uL);
		...
		Debug.Print($"InitializeInADisgustingManner is called. IsValidForTick: {((GauntletOrderUIHandler)this).IsValidForTick}", 0, (DebugColor)12, 17592186044416uL);
		((MissionBehavior)this).AfterStart();
		base._orderTroopPlacer = ((MissionBehavior)this).Mission.GetMissionBehavior<OrderTroopPlacer>();
		OrderTroopPlacer orderTroopPlacer = base._orderTroopPlacer;
		...
		Debug.Print("MissionScreen.OrderFlag has been set (MP)", 0, (DebugColor)12, 17592186044416uL);
		((MissionView)this).MissionScreen.SetOrderFlagVisibility(false);
		MissionPeer.OnTeamChanged += new OnTeamChangedDelegate(TeamChange);
