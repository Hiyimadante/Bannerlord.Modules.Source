...
public class MissionLocationLogic : MissionLogic
{
	...
	public MatrixFrame GetSpawnFrameOfPassage(Location location)
	{
		...
		UsableMachine val = _missionAgentHandler.TownPassageProps.FirstOrDefault((UsableMachine x) => ((Passage)(object)x).ToLocation == location) ?? _missionAgentHandler.DisabledPassages.FirstOrDefault((UsableMachine x) => ((Passage)(object)x).ToLocation == location);
		if (val != null)
		{
