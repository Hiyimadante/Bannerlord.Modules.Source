		...
		for (int i = 0; i < troopSuppliers.Length; i++)
		{
			foreach (IAgentOriginBase item in troopSuppliers[i].SupplyTroops(_numberOfMaxTroopForPlayer + _numberOfMaxTroopForEnemy).ToList())
			{
				...
				if ((!flag || _numberOfMaxTroopForPlayer >= _playerSideSpawnedTroopCount) && (flag || _numberOfMaxTroopForEnemy >= _otherSideSpawnedTroopCount))
				{
					...
					bool flag2 = (item.Troop.IsPlayerCharacter ? true : false);
					((MissionBehavior)this).Mission.SpawnTroop(item, flag, false, flag2, false, 0, 0, true, false, (Vec3?)((WorldPosition)(ref val2.Origin)).GetGroundVec3(), (Vec2?)((Vec3)(ref val2.Rotation.f)).AsVec2, (string)null, (ItemObject)null, (FormationClass)10, false).Defensiveness = 1f;
					if (flag)
					{
