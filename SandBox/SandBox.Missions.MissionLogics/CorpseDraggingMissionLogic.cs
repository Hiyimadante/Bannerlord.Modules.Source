...
public class CorpseDraggingMissionLogic : MissionLogic, IPlayerInputEffector, IMissionBehavior
{
	...
	private void SetDraggedCorpse(Agent draggedCorpse, sbyte draggedCorpseBoneIndex)
	{
		...
		else if (draggedCorpse != null)
		{
			...
			{
				val = equipment[primaryWieldedItemIndex];
				if (!((MissionWeapon)(ref val)).IsEmpty)
				{
					draggedCorpse.DropItem(primaryWieldedItemIndex, (WeaponClass)0);
				}
			}
			if ((int)offhandWieldedItemIndex != -1)
			{
				val = equipment[offhandWieldedItemIndex];
				if (!((MissionWeapon)(ref val)).IsEmpty)
				{
					draggedCorpse.DropItem(offhandWieldedItemIndex, (WeaponClass)0);
				}
			}
		}
