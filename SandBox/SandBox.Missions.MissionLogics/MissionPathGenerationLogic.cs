...
public class MissionPathGenerationLogic : MissionLogic
{
	...
	private List<StandingGuardSpawnData> GetGuardSpawnPoints(NavigationPathData pathData)
	{
		...
		{
			float num = CalculateSpawnGuardScore(validUsableMachinesDatum, out var spawnRotation);
			if (num > 0f)
			{
	...
	private List<VisitPointNodeScoreData> GetVisitPoints(NavigationPathData pathData)
	{
		...
		for (int i = 0; i < path.Size - 1; i++)
		{
			...
			foreach (UsableMachineData validUsableMachinesDatum in pathData.ValidUsableMachinesData)
			{
				...
				{
					float num = CalculateVisitPointScore(validUsableMachinesDatum, path, tempWorldPosition, tempWorldPosition2, out var possibleBlendPointPosition, out var startingAngle, out var pathToVisitPointZero, out var closestPointToPath);
					if (num > 0f)
					{
