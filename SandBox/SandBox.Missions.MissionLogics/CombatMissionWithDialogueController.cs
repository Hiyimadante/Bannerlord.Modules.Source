...
public class CombatMissionWithDialogueController : MissionLogic, IMissionAgentSpawnLogic, IMissionBehavior
{
	...
	public void StartFight(bool hasPlayerChangedSide)
	{
		...
		foreach (Agent item in (List<Agent>)(object)((MissionBehavior)this).Mission.Agents)
		{
			...
			if (hasPlayerChangedSide && item.Team != Mission.Current.PlayerTeam)
			{
				IBattleCombatant battleCombatant = item.Origin.BattleCombatant;
				if ((object)((battleCombatant is PartyBase) ? battleCombatant : null) == PartyBase.MainParty)
				{
	...
	private void SpawnAgents()
	{
		...
		for (int i = 0; i < troopSuppliers.Length; i++)
		{
			...
			{
				Agent val = Mission.Current.SpawnTroop(item, (int)item.BattleCombatant.Side == 1, false, false, false, 0, 0, false, true, (Vec3?)null, (Vec2?)null, (string)null, (ItemObject)null, (FormationClass)10, false);
				_numSpawnedTroops++;
				if (!val.IsMainAgent)
				{
