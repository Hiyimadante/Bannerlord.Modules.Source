...
public class NotableWantsDaughterFoundIssueBehavior : CampaignBehaviorBase
{
	...
	public class NotableWantsDaughterFoundIssue : IssueBase
	{
		...
		public override TextObject IssueBriefByIssueGiver
		{
			get
			{
				...
				val.SetTextVariable("GOLD_ICON", "{=!}<img src=\"General\\Icons\\Coin@2x\" extend=\"6\">");
		...
		public override TextObject IssueAcceptByPlayer
		{
			get
			{
				//IL_0006: Unknown result type (might be due to invalid IL or missing references)
				//IL_000c: Expected O, but got Unknown
				return new TextObject("{=35w6g8gM}Tell me more. What's wrong with the man? ", (Dictionary<string, object>)null);
			}
		}
		...
		public override TextObject IssuePlayerResponseAfterAlternativeExplanation
		{
			get
			{
				//IL_0006: Unknown result type (might be due to invalid IL or missing references)
				//IL_000c: Expected O, but got Unknown
				return new TextObject("{=Ldp6ckgj}Don't worry, either I or one of my companions should be able to find her and see what's going on.", (Dictionary<string, object>)null);
			}
		}
		...
		public override TextObject IssueQuestSolutionAcceptByPlayer
		{
			get
			{
				//IL_0006: Unknown result type (might be due to invalid IL or missing references)
				//IL_000c: Expected O, but got Unknown
				return new TextObject("{=uYrxCtDa}I should be able to find her and see what's going on.", (Dictionary<string, object>)null);
			}
		}
		...
		public override TextObject IssueDiscussAlternativeSolution
		{
			get
			{
				//IL_0006: Unknown result type (might be due to invalid IL or missing references)
				//IL_000c: Expected O, but got Unknown
				return new TextObject("{=mBPcZddA}{?PLAYER.GENDER}Madam{?}Sir{\\?}, we are still waiting [ib:demure][if:convo_undecided_open]for your men to bring my daughter back. I pray for their success.", (Dictionary<string, object>)null);
			}
		}
		...
		public NotableWantsDaughterFoundIssue(Hero issueOwner)
		{
			...
			((IssueBase)this)._002Ector(issueOwner, CampaignTime.DaysFromNow(30f));
		}
	...
	public class NotableWantsDaughterFoundIssueQuest : QuestBase
	{
		...
		public NotableWantsDaughterFoundIssueQuest(string questId, Hero questGiver, CampaignTime duration, int baseReward, float issueDifficultyMultiplier)
		{
			...
			_exitedQuestSettlementForTheFirstTime = true;
			_villagesAndAlreadyVisitedBooleans = new Dictionary<Village, bool>();
			_rogueCharacterBasedOnCulture = new Dictionary<string, CharacterObject>();
			((QuestBase)this)._002Ector(questId, questGiver, duration, baseReward);
			...
			Clan? obj = Clan.BanditFactions.FirstOrDefault((Clan x) => ((MBObjectBase)x).StringId == "steppe_bandits");
			...
			Clan? obj2 = Clan.BanditFactions.FirstOrDefault((Clan x) => ((MBObjectBase)x).StringId == "mountain_bandits");
			...
			Clan? obj3 = Clan.BanditFactions.FirstOrDefault((Clan x) => ((MBObjectBase)x).StringId == "desert_bandits");
			...
			Clan? obj4 = Clan.BanditFactions.FirstOrDefault((Clan x) => ((MBObjectBase)x).StringId == "forest_bandits");
			...
			Clan? obj5 = Clan.BanditFactions.FirstOrDefault((Clan x) => ((MBObjectBase)x).StringId == "sea_raiders");
			...
			Clan? obj6 = Clan.BanditFactions.FirstOrDefault((Clan x) => ((MBObjectBase)x).StringId == "mountain_bandits");
			...
			Clan? obj7 = Clan.BanditFactions.FirstOrDefault((Clan x) => ((MBObjectBase)x).StringId == "mountain_bandits");
			...
			Clan? obj8 = Clan.BanditFactions.FirstOrDefault((Clan x) => ((MBObjectBase)x).StringId == "mountain_bandits");
			...
			Clan? obj9 = Clan.BanditFactions.FirstOrDefault((Clan x) => ((MBObjectBase)x).StringId == "sea_raiders");
		...
		private CharacterObject GetRogueCharacterBasedOnCulture(string cultureStrId)
		{
			...
			return Extensions.GetRandomElementWithPredicate<CharacterObject>(((QuestBase)this).QuestGiver.CurrentSettlement.Culture.NotableTemplates, (Func<CharacterObject, bool>)delegate(CharacterObject x)
			{
				//IL_0001: Unknown result type (might be due to invalid IL or missing references)
				//IL_0008: Invalid comparison between Unknown and I4
				return (int)x.Occupation == 21 && !((BasicCharacterObject)x).IsFemale;
			});
		}
		...
		private void OnSettlementEntered(MobileParty party, Settlement settlement, Hero hero)
		{
			...
			if (_villagesAndAlreadyVisitedBooleans.ContainsKey(settlement.Village) && !_villagesAndAlreadyVisitedBooleans[settlement.Village])
			{
				...
				{
					if (settlement.IsSettlementBusy((object)this))
					{
	...
	{
		if (issueGiver.IsRuralNotable && issueGiver.CurrentSettlement.IsVillage && issueGiver.CurrentSettlement.Village.Bound != null && ((List<Village>)(object)issueGiver.CurrentSettlement.Village.Bound.BoundVillages).Count > 2 && issueGiver.CanHaveCampaignIssues() && issueGiver.Age > (float)(Campaign.Current.Models.AgeModel.HeroComesOfAge * 2) && CharacterHelper.GetRandomCompanionTemplateWithPredicate((Func<CharacterObject, bool>)((CharacterObject x) => ((BasicCharacterObject)x).IsFemale && x.Culture == issueGiver.CurrentSettlement.Culture)) != null && ((IEnumerable<CharacterObject>)issueGiver.CurrentSettlement.Culture.NotableTemplates).Any(delegate(CharacterObject x)
		{
			//IL_0001: Unknown result type (might be due to invalid IL or missing references)
			//IL_0008: Invalid comparison between Unknown and I4
			return (int)x.Occupation == 21 && !((BasicCharacterObject)x).IsFemale;
		}) && issueGiver.GetTraitLevel(DefaultTraits.Mercy) <= 0)
		{
