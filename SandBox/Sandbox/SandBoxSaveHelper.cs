...
public static class SandBoxSaveHelper
{
	...
	public static void TryLoadSave(SaveGameFileInfo saveInfo, Action<LoadResult> onStartGame, Action onCancel = null)
	{
		...
		OnStateChange?.Invoke(SaveHelperState.Start);
		...
		{
			IEnumerable<IGrouping<ModuleCheckResultType, ModuleCheckResult>> enumerable = ((IEnumerable<ModuleCheckResult>)val).GroupBy(delegate(ModuleCheckResult m)
			{
			...
			OnStateChange?.Invoke(SaveHelperState.Inquiry);
		}
		else
		{
			LoadGameAction(saveInfo, onStartGame, onCancel);
	...
	private static void LoadGameAction(SaveGameFileInfo saveInfo, Action<LoadResult> onStartGame, Action onCancel)
	{
		...
		OnStateChange?.Invoke(SaveHelperState.LoadGame);
	...
	static SandBoxSaveHelper()
	{
		...
		SaveResetVersion = new ApplicationVersion((ApplicationVersionType)2, 1, 7, 0, 0);
		_moduleMismatchInquiryTitle = new TextObject("{=r7xdYj4q}Module Mismatch", (Dictionary<string, object>)null);
		_errorTitle = new TextObject("{=oZrVNUOk}Error", (Dictionary<string, object>)null);
		_saveLoadingProblemText = new TextObject("{=onLDP7mP}A problem occured while trying to load the saved game.", (Dictionary<string, object>)null);
		_saveResetVersionProblemText = new TextObject("{=5hbSkbQg}This save file is from a game version that is older than e1.7.0. Please switch your game version to e1.7.0, load the save file and save the game. This will allow it to work on newer versions beyond e1.7.0.", (Dictionary<string, object>)null);
	}
}
