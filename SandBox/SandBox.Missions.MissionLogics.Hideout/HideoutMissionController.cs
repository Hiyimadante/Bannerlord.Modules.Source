...
public class HideoutMissionController : MissionLogic, IMissionAgentSpawnLogic, IMissionBehavior
{
	private class MissionSide
	{
		...
		public void SpawnTroops(List<CommonAreaMarker> areaMarkers, List<PatrolArea> patrolAreas, Dictionary<Agent, UsedObject> defenderAgentObjects, int spawnCount)
		{
			...
			for (int i = 0; i < list2.Count; i++)
			{
				if (1 == (int)_side)
				{
					Mission.Current.SpawnTroop(list2[i], true, true, false, false, 0, 0, true, true, (Vec3?)null, (Vec2?)null, (string)null, (ItemObject)null, (FormationClass)10, false);
				...
				else
				{
					...
					if (!flag)
					{
						...
						val = list.FirstOrDefault((StandingPoint point) => !((UsableMissionObject)point).IsDeactivated && !((MissionObject)point).IsDisabled && !((UsableMissionObject)point).HasUser);
					}
					else
					{
					...
					if (val != null && !((MissionObject)val).IsDisabled)
					{
						...
						Agent agent = current3.SpawnTroop(obj, false, false, false, false, 0, 0, false, false, val3, (Vec2?)((Vec2)(ref asVec)).Normalized(), "_hideout_bandit", (ItemObject)null, (FormationClass)10, false);
		...
		public void SpawnRemainingTroopsForBossFight(List<MatrixFrame> spawnFrames, int spawnCount, CharacterObject overriddenHideoutBossCharacterObject)
		{
			...
			if (overriddenHideoutBossCharacterObject != null)
			{
				...
				Agent val4 = current.SpawnTroop(val, false, false, false, false, 0, 0, false, false, val3, (Vec2?)((Vec2)(ref asVec)).Normalized(), "_hideout_bandit", (ItemObject)null, (FormationClass)10, false);
				_numberOfSpawnedTroops++;
			...
			for (int num = 0; num < list.Count; num++)
			{
				...
				Agent val7 = current2.SpawnTroop(obj, false, false, false, false, 0, 0, false, false, val6, (Vec2?)((Vec2)(ref asVec)).Normalized(), "_hideout_bandit", (ItemObject)null, (FormationClass)10, false);
	...
	public override void OnBehaviorInitialize()
	{
		...
		((MissionBehavior)this).Mission.GetAgentTroopClass_Override += GetHideoutMissionTroopClass;
	}
		...
		if (!flag)
		{
			if ((Agent.Main == null || !Agent.Main.IsActive()) && (int)side == 1)
			{
				...
				else if (_hideoutMissionState == HideoutMissionState.WithoutBossFight || _hideoutMissionState == HideoutMissionState.BossFightWithAll)
				{
					...
					bool flag2 = ((IEnumerable<Agent>)((MissionBehavior)this).Mission.Teams.Defender.ActiveAgents).Any(delegate(Agent t)
					{
						//IL_0001: Unknown result type (might be due to invalid IL or missing references)
						//IL_0007: Invalid comparison between Unknown and I4
						return (int)t.CurrentWatchState == 2;
					});
					flag = !num && !flag2;
				}
			}
