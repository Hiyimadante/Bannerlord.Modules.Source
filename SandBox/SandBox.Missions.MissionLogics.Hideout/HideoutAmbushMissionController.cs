...
public class HideoutAmbushMissionController : MissionLogic
{
	...
	private bool _isClearedAsGhost;
	...
	private readonly List<Agent> _clearObjectiveTargetAgents;
	...
	public HideoutAmbushMissionController(IMissionTroopSupplier[] suppliers, BattleSideEnum playerSide, int playerTroopCount)
	{
		...
		_isClearedAsGhost = true;
		_clearObjectiveTargetAgents = new List<Agent>();
	...
	public override void OnBehaviorInitialize()
	{
		...
		((MissionBehavior)this).Mission.GetAgentTroopClass_Override += GetHideoutAmbushMissionTroopClass;
	}
	...
	private void SpawnRemainingTroopsForBossFight(List<MatrixFrame> spawnFrames, int spawnCount)
	{
		...
		if (_overriddenHideoutBossAgentOrigin != null)
		{
			...
			Agent val3 = current.SpawnTroop(overriddenHideoutBossAgentOrigin, false, false, false, false, 0, 0, false, false, val2, (Vec2?)((Vec2)(ref asVec)).Normalized(), "_hideout_bandit", (ItemObject)null, (FormationClass)10, false);
		...
		for (int j = 0; j < _allEnemyTroops.Count; j++)
		{
			...
			Agent val6 = current2.SpawnTroop(obj, false, false, false, false, 0, 0, false, false, val5, (Vec2?)((Vec2)(ref asVec)).Normalized(), "_hideout_bandit", (ItemObject)null, (FormationClass)10, false);
	...
	private Agent SpawnAllyAgent(IAgentOriginBase character, GameEntity spawnPoint, Vec3 position)
	{
		...
		Agent obj = current.SpawnTroop(character, true, false, false, false, 0, 0, true, true, val, (Vec2?)((Vec2)(ref asVec)).Normalized(), (string)null, (ItemObject)null, (FormationClass)10, false);
	...
	private void InitializeTroops()
	{
		...
		_overriddenHideoutBossAgentOrigin = source.FirstOrDefault((IAgentOriginBase x) => (object)x.Troop == _overriddenHideoutBossCharacterObject);
		_allEnemyTroops = source.Where(delegate(IAgentOriginBase x)
		{
			if (!x.Troop.IsHero)
			{
				BasicCharacterObject troop = x.Troop;
				CharacterObject val;
				if ((val = (CharacterObject)(object)((troop is CharacterObject) ? troop : null)) != null && val.Culture.BanditBoss != val)
				{
					return val != _overriddenHideoutBossCharacterObject;
				}
			}
			return false;
		}).ToList();
