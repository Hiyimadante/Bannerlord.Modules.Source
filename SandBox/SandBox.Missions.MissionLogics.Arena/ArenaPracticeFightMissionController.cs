...
public class ArenaPracticeFightMissionController : MissionLogic
{
	...
	public override void AfterStart()
	{
		...
		_initialSpawnFrames = ((MissionBehavior)this).Mission.Scene.FindEntitiesWithTag("sp_arena").Select(delegate(GameEntity e)
		{
			//IL_0001: Unknown result type (might be due to invalid IL or missing references)
			return e.GetGlobalFrame();
		}).ToList();
		_spawnFrames = ((MissionBehavior)this).Mission.Scene.FindEntitiesWithTag("sp_arena_respawn").Select(delegate(GameEntity e)
		{
			//IL_0001: Unknown result type (might be due to invalid IL or missing references)
			return e.GetGlobalFrame();
		}).ToList();
	...
	public override void OnMissionTick(float dt)
	{
		...
		if (_teleportTimer != null && _teleportTimer.ElapsedTime > (float)TeleportTime)
		{
			...
			Agent agent = ((IEnumerable<Agent>)((MissionBehavior)this).Mission.Agents).FirstOrDefault(delegate(Agent x)
			{
				//IL_000e: Unknown result type (might be due to invalid IL or missing references)
				//IL_0013: Unknown result type (might be due to invalid IL or missing references)
				//IL_0019: Invalid comparison between Unknown and I4
				return x.Character != null && (int)((CharacterObject)x.Character).Occupation == 5;
			});
