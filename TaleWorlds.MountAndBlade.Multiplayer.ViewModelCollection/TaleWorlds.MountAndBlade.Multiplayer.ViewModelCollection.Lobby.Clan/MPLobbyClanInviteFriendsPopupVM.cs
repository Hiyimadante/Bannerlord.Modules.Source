...
public class MPLobbyClanInviteFriendsPopupVM : ViewModel
{
	...
	public void Open()
	{
		...
		IEnumerable<PlayerId> source = NetworkMain.GameClient.ClanInfo.Players.Select(delegate(ClanPlayer c)
		{
		...
		{
			if (!source.Contains(onlineFriend.ProvidedID) && !((IEnumerable<MPLobbyPlayerBaseVM>)OnlineFriends).Any(delegate(MPLobbyPlayerBaseVM f)
			{
				//IL_0001: Unknown result type (might be due to invalid IL or missing references)
				//IL_000c: Unknown result type (might be due to invalid IL or missing references)
				return f.ProvidedID == onlineFriend.ProvidedID;
			}))
			{
