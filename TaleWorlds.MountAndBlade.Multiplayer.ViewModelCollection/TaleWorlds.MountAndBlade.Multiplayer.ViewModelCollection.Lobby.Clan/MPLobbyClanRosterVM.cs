		...
		if (NetworkMain.GameClient.PlayerID != member.Id)
		{
			if (_isClanLeader)
			{
				...
				else
				{
					...
					if (NetworkMain.GameClient.PlayersInClan.Count(delegate(ClanPlayer m)
					{
						//IL_0001: Unknown result type (might be due to invalid IL or missing references)
						//IL_0007: Invalid comparison between Unknown and I4
						return (int)m.Role == 1;
					}) == Parameters.ClanOfficerCount)
					{
						val.IsEnabled = false;
			...
			if (NetworkMain.GameClient.FriendInfos.All(delegate(FriendInfo f)
			{
				//IL_0001: Unknown result type (might be due to invalid IL or missing references)
