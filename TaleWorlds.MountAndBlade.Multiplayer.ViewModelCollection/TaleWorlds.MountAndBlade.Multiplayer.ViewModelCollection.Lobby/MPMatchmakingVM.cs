...
public class MPMatchmakingVM : ViewModel
{
	...
	public void RefreshPlayerData(PlayerData playerData)
	{
		...
		if (lastGameTypes != null)
		{
			...
			{
				item.IsSelected = Enumerable.Contains(lastGameTypes, item.Type);
			}
			return;
		}
	...
	{
		bool selectedGameTypesInfo = GetSelectedGameTypesInfo(out var gameTypes);
	...
	private void UpdateRankedGameTypesList()
	{
		...
		for (int i = 0; i < availableRankedGameModes.Length; i++)
		{
			...
			mPMatchmakingItemVM.IsSelected = Enumerable.Contains(_defaultSelectedGameTypes, mPMatchmakingItemVM.Type);
