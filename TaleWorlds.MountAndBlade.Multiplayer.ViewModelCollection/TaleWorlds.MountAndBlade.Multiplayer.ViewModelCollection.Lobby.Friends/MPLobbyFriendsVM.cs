...
public class MPLobbyFriendsVM : ViewModel
{
	...
	private PlayerId? _partyLeaderId
	{
		get
		{
			...
			PartyPlayerInLobbyClient? obj = NetworkMain.GameClient.PlayersInParty.SingleOrDefault((PartyPlayerInLobbyClient p) => p.IsPartyLeader);
			if (obj == null)
			{
				return null;
			}
	...
	internal void OnClanInfoChanged()
	{
		...
		{
			IFriendListService[] friendListServices = PlatformServices.Instance.GetFriendListServices();
			IFriendListService val = friendListServices.FirstOrDefault((IFriendListService f) => f.GetServiceCodeName() == "ClanFriends");
			if (val != null)
			{
	...
	private void ActivatePartyPlayerActions(MPLobbyPartyPlayerVM player)
	{
		...
		{
			PartyPlayerInLobbyClient? obj = NetworkMain.GameClient.PlayersInParty.SingleOrDefault(delegate(PartyPlayerInLobbyClient p)
			{
				//IL_0001: Unknown result type (might be due to invalid IL or missing references)
				//IL_000c: Unknown result type (might be due to invalid IL or missing references)
				return p.PlayerId == player.ProvidedID;
			});
