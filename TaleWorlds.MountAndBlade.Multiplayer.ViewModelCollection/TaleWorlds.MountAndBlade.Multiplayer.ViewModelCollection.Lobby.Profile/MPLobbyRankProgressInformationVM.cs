...
public class MPLobbyRankProgressInformationVM : ViewModel
{
	...
	public MPLobbyRankProgressInformationVM(Func<string> getExitText)
	{
		...
		_ratingRemainingTitleTextObject = new TextObject("{=7gQkFJqA}{RATING} points remaining to next rank", (Dictionary<string, object>)null);
		_finalRankTextObject = new TextObject("{=6mZymVS8}You are at the final rank", (Dictionary<string, object>)null);
		_evaluationTextObject = new TextObject("{=Ise5gWw3}{PLAYED_GAMES} / {TOTAL_GAMES} Evaluation matches played", (Dictionary<string, object>)null);
	...
	private void RefreshRankInfo(string gameType)
	{
		...
		GameTypeRankInfo val = Player.RankInfo?.FirstOrDefault((GameTypeRankInfo r) => r.GameType == gameType);
		if (val != null && val.RankBarInfo != null)
		{
			...
			StringPairItemVM val2 = ((IEnumerable<StringPairItemVM>)AllRanks).FirstOrDefault((StringPairItemVM r) => r.Definition == CurrentRankID);
			if (val2 != null)
			{
