...
public class MPHostGameOptionsVM : ViewModel
{
	...
	private void OnGameModeChanged(string gameModeName)
	{
		...
		(((IEnumerable<GenericHostGameOptionDataVM>)GeneralOptions).First(delegate(GenericHostGameOptionDataVM o)
		{
			//IL_0001: Unknown result type (might be due to invalid IL or missing references)
			//IL_0008: Invalid comparison between Unknown and I4
			return (int)o.OptionType == 13;
		}) as MultipleSelectionHostGameOptionDataVM)?.RefreshList();
	...
	private void FillOptionsForCustomServer(string gameModeName)
	{
		...
		for (OptionType val = (OptionType)0; (int)val < 43; val = (OptionType)(val + 1))
		{
			...
			if (optionProperty.ValidGameModes != null && Enumerable.Contains(optionProperty.ValidGameModes, gameModeName))
			{
				flag = true;
			}
