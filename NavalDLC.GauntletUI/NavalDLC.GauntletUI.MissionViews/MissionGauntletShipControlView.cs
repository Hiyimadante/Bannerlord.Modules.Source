	...
	private void TickInput()
	{
		...
		if (val.IsGameKeyReleased(114) && GetCanAttemptBoarding())
		{
			...
			else if (!GetIsAttemptBoardingTemporarilyBlocked())
			{
				...
				DisplayCommandForSelectedFormations(new TextObject("{=HSALr4nl}Board {SHIP_NAME}!", (Dictionary<string, object>)null).SetTextVariable("SHIP_NAME", (_focusedShip.Team == null || (int)_focusedShip.Team.TeamSide == 2) ? _focusedShip.ShipOrigin.Hull.Name : _focusedShip.ShipOrigin.Name));
			}
		}
