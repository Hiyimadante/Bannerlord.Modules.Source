...
public class MissionGauntletOptionsUIHandler : MissionView
{
	...
	private void OnShowOptions()
	{
		...
		_initialClothSimValue = NativeOptions.GetConfig((NativeOptionsType)53) == 0f;
	...
	private void OnCloseOptions()
	{
		...
		bool flag = NativeOptions.GetConfig((NativeOptionsType)53) == 0f;
	...
	private void SetHotKey(Key key)
	{
		...
		if ((gameKey = (GameKeyOptionVM)(object)((currentKey is GameKeyOptionVM) ? currentKey : null)) != null)
		{
			if (((IEnumerable<GameKeyGroupVM>)_dataSource.GameKeyOptionGroups.GameKeyGroups).FirstOrDefault((GameKeyGroupVM g) => ((Collection<GameKeyOptionVM>)(object)g.GameKeys).Contains(gameKey)) == null)
			{
			...
			if (((ScreenLayer)_gauntletLayer).Input.IsHotKeyReleased("Exit") || key.InputKey == ((KeyOptionVM)gameKey).CurrentKey.InputKey)
			{
				_keybindingPopup.OnToggle(isActive: false);
			}
			else
		...
		else
		{
			...
			if ((auxiliaryKey = (AuxiliaryKeyOptionVM)(object)((currentKey2 is AuxiliaryKeyOptionVM) ? currentKey2 : null)) != null)
			{
				if (((IEnumerable<AuxiliaryKeyGroupVM>)_dataSource.GameKeyOptionGroups.AuxiliaryKeyGroups).FirstOrDefault((AuxiliaryKeyGroupVM g) => ((Collection<AuxiliaryKeyOptionVM>)(object)g.HotKeys).Contains(auxiliaryKey)) == null)
				{
				...
				if (((ScreenLayer)_gauntletLayer).Input.IsHotKeyReleased("Exit") || key.InputKey == ((KeyOptionVM)auxiliaryKey).CurrentKey.InputKey)
				{
					_keybindingPopup.OnToggle(isActive: false);
				}
				else
		...
		if (dataSource != null)
		{
			...
			{
				((ViewModel)gameKeyOptionGroups).RefreshValues();
			}
		}
	}
}
