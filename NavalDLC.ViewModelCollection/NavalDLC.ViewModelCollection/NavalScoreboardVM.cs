...
public class NavalScoreboardVM : SPScoreboardVM
{
	private class ScoreboardShipComparer : IComparer<SPScoreboardShipVM>
	{
		...
		private int ResolveEquality(SPScoreboardShipVM x, SPScoreboardShipVM y)
		{
			...
			int value = ((Ship)((ship is Ship) ? ship : null)).ShipHull.Value;
			IShipOrigin ship2 = x.Ship;
			return value.CompareTo(((Ship)((ship2 is Ship) ? ship2 : null)).ShipHull.Value);
		}
	}
	...
	public override void Initialize(IMissionScreen missionScreen, Mission mission, Action releaseSimulationSources, Action<bool> onToggle)
	{
		...
		if (((ScoreboardBaseVM)this).IsSimulation)
		{
			...
			MapEvent val = ((mainParty != null) ? mainParty.MapEvent : null);
			if (val == null || (!val.IsNavalMapEvent && !MapEventHelper.IsNavalRaid(val)))
			{
		...
		if (((ScoreboardBaseVM)this).IsSimulation)
		{
			...
			using (List<Ship>.Enumerator enumerator = ((List<Ship>)(object)MobileParty.MainParty.MapEvent.AttackerSide.SimulationShipList).GetEnumerator())
			{
				Ship current2;
				TeamSideEnum val2;
				for (; enumerator.MoveNext(); ((ScoreboardBaseVM)this).Attackers.GetShipAddIfNotExists((IShipOrigin)(object)current2, ((object)current2.ShipHull.Type/*cast due to constrained. prefix*/).ToString(), (IBattleCombatant)(object)current2.Owner, val2))
				{
			...
			if (!MapEventHelper.IsNavalRaid((mainParty2 != null) ? mainParty2.MapEvent : null))
			{
				...
				Ship current3;
				TeamSideEnum val3;
				for (; enumerator.MoveNext(); ((ScoreboardBaseVM)this).Defenders.GetShipAddIfNotExists((IShipOrigin)(object)current3, ((object)current3.ShipHull.Type/*cast due to constrained. prefix*/).ToString(), (IBattleCombatant)(object)current3.Owner, val3))
				{
	...
	protected override void OnTick(float dt)
	{
		...
		else
		{
			...
			for (int k = 0; k < ((Collection<SPScoreboardShipVM>)(object)((ScoreboardBaseVM)this).Attackers.Ships).Count; k++)
			{
				...
				bool flag = _navalShipsLogic.FindAssignmentOfShipOrigin(val.Ship, out var shipAssignment);
			...
			for (int l = 0; l < ((Collection<SPScoreboardShipVM>)(object)((ScoreboardBaseVM)this).Defenders.Ships).Count; l++)
			{
				...
				bool flag2 = _navalShipsLogic.FindAssignmentOfShipOrigin(val2.Ship, out var shipAssignment2);
	...
	private void UpdateTeamShips(bool removeOld, bool addNew, bool sort)
	{
		...
		if (addNew)
		{
			...
			for (int i = 0; i < ((List<MissionShip>)(object)val).Count; i++)
			{
				...
				SPScoreboardSideVM attackers = ((ScoreboardBaseVM)this).Attackers;
				...
				attackers.GetShipAddIfNotExists(shipOrigin, text, (IBattleCombatant)(object)((Ship)((shipOrigin2 is Ship) ? shipOrigin2 : null)).Owner, Mission.Current.AttackerTeam.TeamSide);
			}
			for (int j = 0; j < ((List<MissionShip>)(object)val2).Count; j++)
			{
				MissionShip missionShip2 = ((List<MissionShip>)(object)val2)[j];
				SPScoreboardSideVM attackers2 = ((ScoreboardBaseVM)this).Attackers;
				...
				attackers2.GetShipAddIfNotExists(shipOrigin3, text2, (IBattleCombatant)(object)((Ship)((shipOrigin4 is Ship) ? shipOrigin4 : null)).Owner, Mission.Current.AttackerAllyTeam.TeamSide);
			}
			for (int k = 0; k < ((List<MissionShip>)(object)val3).Count; k++)
			{
				MissionShip missionShip3 = ((List<MissionShip>)(object)val3)[k];
				SPScoreboardSideVM defenders = ((ScoreboardBaseVM)this).Defenders;
				...
				defenders.GetShipAddIfNotExists(shipOrigin5, text3, (IBattleCombatant)(object)((Ship)((shipOrigin6 is Ship) ? shipOrigin6 : null)).Owner, Mission.Current.DefenderTeam.TeamSide);
			}
			for (int l = 0; l < ((List<MissionShip>)(object)val4).Count; l++)
			{
				MissionShip missionShip4 = ((List<MissionShip>)(object)val4)[l];
				SPScoreboardSideVM defenders2 = ((ScoreboardBaseVM)this).Defenders;
				...
				defenders2.GetShipAddIfNotExists(shipOrigin7, text4, (IBattleCombatant)(object)((Ship)((shipOrigin8 is Ship) ? shipOrigin8 : null)).Owner, Mission.Current.DefenderAllyTeam.TeamSide);
			}
		}
		if (sort)
		{
