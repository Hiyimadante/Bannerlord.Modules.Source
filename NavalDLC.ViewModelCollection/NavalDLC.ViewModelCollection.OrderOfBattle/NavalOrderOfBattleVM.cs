...
public class NavalOrderOfBattleVM : ViewModel
{
	...
	public NavalOrderOfBattleVM(Mission mission, Action<NavalOrderOfBattleFormationItemVM> onFormationSelected, Action clearFormationSelection, Action onAutoDeploy, Action onBeginMission)
	{
		...
		_formationsDisabledHintGeneral = new TextObject("{=ZixS1b4u}You're not leading this battle.", (Dictionary<string, object>)null);
		_formationsDisabledHintAllies = new TextObject("{=O4n4SAqo}Formation is reserved for allied parties.", (Dictionary<string, object>)null);
		_formationsDisabledHintSkills = new TextObject("{=Vs5NavCd}You do not have enough skills/perks for this formation.", (Dictionary<string, object>)null);
		_formationsDisabledHintShips = new TextObject("{=bID6axoH}You do not have enough ships for this formation.", (Dictionary<string, object>)null);
	...
	public void ExecuteBeginMission()
	{
		...
		foreach (NavalOrderOfBattleFormationItemVM item in (List<NavalOrderOfBattleFormationItemVM>)(object)AllFormations)
		{
			...
			{
				CurrentFilterConfiguration?.Add(new FormationConfiguration(item.Formation.Index, ((IEnumerable<OrderOfBattleFormationFilterSelectorItemVM>)item.FilterItems).Where((OrderOfBattleFormationFilterSelectorItemVM f) => f.IsActive).Select(delegate(OrderOfBattleFormationFilterSelectorItemVM f)
				{
	...
	private void RefreshShips()
	{
		...
		for (int i = 0; i < _allShips.Count; i++)
		{
			...
			bool flag = _navalShipsLogic.FindAssignmentOfShipOrigin(navalOrderOfBattleShipItemVM.ShipOrigin, out var shipAssignment);
			int isDisabled;
			if (!IsPlayerGeneral)
			{
	...
	private void RefreshFormationsDisabledAndReason()
	{
		...
		int shipDeploymentLimit = _navalShipsLogic.GetShipDeploymentLimit((TeamSideEnum)1, out var _);
	...
	private void LoadConfigurationAgents()
	{
		...
		for (int i = 0; i < ((List<NavalOrderOfBattleFormationItemVM>)(object)AllFormations).Count; i++)
		{
			...
			if ((int)formationInfo.FormationClass != 0 && navalOrderOfBattleFormationItemVM.IsSelectable)
			{
				...
				((IEnumerable<OrderOfBattleFormationFilterSelectorItemVM>)navalOrderOfBattleFormationItemVM.FilterItems).FirstOrDefault(delegate(OrderOfBattleFormationFilterSelectorItemVM f)
				{
					//IL_0001: Unknown result type (might be due to invalid IL or missing references)
	...
	private void OnFilterUseToggled(NavalOrderOfBattleFormationItemVM formationItem)
	{
		...
		if (!IsAssignmentDirty)
		{
			TroopTraitsMask filter = TroopFilteringUtilities.GetFilter(((IEnumerable<OrderOfBattleFormationFilterSelectorItemVM>)formationItem.FilterItems).Where((OrderOfBattleFormationFilterSelectorItemVM f) => f.IsActive).Select(delegate(OrderOfBattleFormationFilterSelectorItemVM f)
			{
				//IL_0001: Unknown result type (might be due to invalid IL or missing references)
				return f.FilterType;
			}).ToArray());
