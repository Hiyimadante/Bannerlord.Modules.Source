...
public class NavalMissionOrderTroopControllerVM : MissionOrderTroopControllerVM
{
	...
	public void OnClassesSet(List<ClassConfiguration> classData)
	{
		...
		{
			if (((IEnumerable<OrderTroopItemVM>)((MissionOrderTroopControllerVM)this).TroopList).FirstOrDefault((OrderTroopItemVM f) => f.Formation.Index == classItem.FormationIndex) is NavalOrderTroopItemVM navalOrderTroopItemVM)
			{
				navalOrderTroopItemVM.UpdateClassData(classItem.FormationClass);
			}
			if (((IEnumerable<OrderTroopItemVM>)((MissionOrderTroopControllerVM)this).TransferTargetList).FirstOrDefault((OrderTroopItemVM f) => f.Formation.Index == classItem.FormationIndex) is NavalOrderTroopItemVM navalOrderTroopItemVM2)
			{
	...
	public override void SelectAllFormations(bool uiFeedback = true)
	{
		...
		{
			((MissionOrderTroopControllerVM)this).OnSelectFormation(((IEnumerable<OrderTroopItemVM>)((MissionOrderTroopControllerVM)this).TroopList).FirstOrDefault((OrderTroopItemVM x) => ((OrderSubjectVM)x).IsSelectable));
			return;
		}
