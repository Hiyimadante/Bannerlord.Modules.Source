...
public class ShipFigureheadSlotVM : ShipUpgradeSlotBaseVM
{
	...
	{
		Figurehead currentFigurehead = GetCurrentFigurehead?.Invoke(Ship);
		...
		bool isRightSide = GetIsRightSide?.Invoke(Ship) ?? true;
		for (int num = 0; num < ((Collection<ShipUpgradePieceBaseVM>)(object)base.AvailablePieces).Count; num++)
		{
			...
			shipFigureheadVM.EquippedShip = GetShipOfFigurehead?.Invoke(shipFigureheadVM.Figurehead, isRightSide);
