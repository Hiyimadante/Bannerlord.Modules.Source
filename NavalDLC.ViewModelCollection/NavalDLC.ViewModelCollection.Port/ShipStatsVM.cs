...
public class ShipStatsVM : ViewModel
{
	...
	private string GetSailTypeStr(MissionShipObject missionShipObject)
	{
		...
		{
			bool flag = ((IEnumerable<ShipSail>)missionShipObject.Sails).Any(delegate(ShipSail x)
			{
				//IL_0001: Unknown result type (might be due to invalid IL or missing references)
				//IL_0007: Invalid comparison between Unknown and I4
				return (int)x.Type == 1;
			});
			bool flag2 = ((IEnumerable<ShipSail>)missionShipObject.Sails).Any(delegate(ShipSail x)
			{
				//IL_0001: Unknown result type (might be due to invalid IL or missing references)
				//IL_0007: Invalid comparison between Unknown and I4
				return (int)x.Type == 0;
