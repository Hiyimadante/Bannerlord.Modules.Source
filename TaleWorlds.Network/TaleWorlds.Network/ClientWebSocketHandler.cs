...
public class ClientWebSocketHandler
{
	...
	private async Task Receive(ClientWebSocket webSocket)
	{
		...
		while (webSocket.State == WebSocketState.Open)
		{
			try
			{
				...
				if (webSocketReceiveResult.MessageType == WebSocketMessageType.Close)
				{
					try
					{
						...
						{
							await Disconnected(this, onDisconnectCommand: true);
						}
				...
				if (webSocketReceiveResult.EndOfMessage)
				{
					...
					{
						MessageReceived(webSocketMessage, this);
					}
					ms.Seek(0L, SeekOrigin.Begin);
				}
			}
			catch (WebSocketException)
			{
				if (Disconnected != null)
				{
					await Disconnected(this, onDisconnectCommand: false);
				}
				return;
			}
		}
	...
	public async Task Disconnect(string reason, bool onDisconnectCommand)
	{
		...
		if (Disconnected != null)
		{
			await Disconnected(this, onDisconnectCommand);
		}
	}
