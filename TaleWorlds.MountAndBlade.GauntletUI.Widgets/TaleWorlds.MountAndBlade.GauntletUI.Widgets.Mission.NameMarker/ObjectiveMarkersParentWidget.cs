...
public class ObjectiveMarkersParentWidget : Widget
{
	...
	private void UpdateMarkerCombinations()
	{
		...
		for (int j = 0; j < _markers.Count; j++)
		{
			...
			{
				List<ObjectiveMarkerWidget> list = FindClosestMarkersToCombine(objectiveMarkerWidget, out var averageScreenPosition);
	...
	{
		ObjectiveMarkerWidget objectiveMarkerWidget = FindClosestMarkerToFocus(base.EventManager.PageSize * 0.5f, out var _);
