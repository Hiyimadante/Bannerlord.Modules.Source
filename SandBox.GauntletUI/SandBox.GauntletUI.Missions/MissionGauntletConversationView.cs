...
public class MissionGauntletConversationView : MissionView, IConversationStateHandler
{
	...
	void IConversationStateHandler.OnConversationInstall()
	{
		...
		if (((MissionBehavior)this).Mission.HasMissionBehavior<ConversationMissionLogic>())
		{
			((MissionView)this).MissionScreen.SetAsConversationMission();
		}
		Campaign.Current.ConversationManager.ConversationEndOneShot += OnConversationEndOneShot;
	...
	{
		if (((MissionView)this).MissionScreen.IsConversationMission)
		{
			((View)((MissionView)this).MissionScreen.SceneView).SetEnable(false);
