...
public class NavalCustomBattleShipSelectionGroupVM : ViewModel
{
	...
	public bool IsRaid
	{
		...
		set
		{
			if (value != _isRaid)
			{
				...
				{
					((Collection<NavalCustomBattleShipSelectionItemVM>)(object)ShipSelectionItems)[i].IsRaid = value;
					((Collection<NavalCustomBattleShipSelectionItemVM>)(object)ShipSelectionItems)[i].IsRelevant = !value || i < 3;
				}
			}
		}
	}
	...
	public void ExecuteRandomize(int targetDeckSize)
	{
		...
		for (int i = 0; i < 20; i++)
		{
			List<ShipHull> list = CreateRandomFleet(targetDeckSize, out var deckSize);
	...
	private List<ShipHull> CreateRandomFleet(int targetDeckSize, out int deckSize)
	{
		...
		deckSize = 0;
		for (int i = 0; i < ((IEnumerable<NavalCustomBattleShipSelectionItemVM>)ShipSelectionItems).Count((NavalCustomBattleShipSelectionItemVM x) => x.IsRelevant); i++)
		{
			ShipHull val = (IsRaid ? Extensions.GetRandomElementWithPredicate<ShipHull>(NavalCustomBattleData.ShipHulls.ToArray(), (Func<ShipHull, bool>)((ShipHull x) => NavalCustomBattleHelper.CanShipHullBeUsedInRaid(x))) : Extensions.GetRandomElementInefficiently<ShipHull>(NavalCustomBattleData.ShipHulls));
			list.Add(val);
			deckSize += val.MainDeckCrewCapacity;
	...
	public List<IShipOrigin> GetSelectedShips()
	{
		...
		{
			if (item.IsRelevant && item.HasSelectedItem)
			{
	...
	{
		int totalSelectedItemCount = ((IEnumerable<NavalCustomBattleShipSelectionItemVM>)ShipSelectionItems).Count((NavalCustomBattleShipSelectionItemVM x) => x.IsRelevant && x.HasSelectedItem);
		ShipSelectionItems.ApplyActionOnAllItems((Action<NavalCustomBattleShipSelectionItemVM>)delegate(NavalCustomBattleShipSelectionItemVM x)
		{
