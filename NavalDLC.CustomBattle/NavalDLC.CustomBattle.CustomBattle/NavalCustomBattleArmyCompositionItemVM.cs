...
public class NavalCustomBattleArmyCompositionItemVM : ViewModel
{
	...
	private void PopulateTroopTypes()
	{
		...
		foreach (BasicCharacterObject allCharacterObject in _allCharacterObjects)
		{
			...
			{
				((Collection<NavalCustomBattleTroopTypeVM>)(object)TroopTypes).Add(new NavalCustomBattleTroopTypeVM(allCharacterObject, _troopTypeSelectionPopUp.OnItemSelectionToggled, GetTroopTypeIconData(allCharacterObject, _type, IsLand), _allSkills, ((List<BasicCharacterObject>)(object)defaultCharacters).Contains(allCharacterObject)));
			}
		}
		IsValid = ((Collection<NavalCustomBattleTroopTypeVM>)(object)TroopTypes).Count > 0;
