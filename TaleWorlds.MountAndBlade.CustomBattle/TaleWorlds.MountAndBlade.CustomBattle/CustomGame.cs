	...
	private void LoadCustomBattleScenes(XmlDocument doc)
	{
		...
		foreach (XmlNode childNode in xmlNode.ChildNodes)
		{
			...
			foreach (XmlNode item in childNodes)
			{
				...
				{
					if (childNode2.NodeType != XmlNodeType.Comment && childNode2.Attributes["name"].InnerText == "TerrainType" && Enum.TryParse<TerrainType>(childNode2.Attributes["value"].InnerText, out TerrainType result8) && !list.Contains(result8))
					{
						list.Add(result8);
					}
				}
