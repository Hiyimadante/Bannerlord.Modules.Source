...
public class ProfanityChecker
{
	...
	public bool IsProfane(string word)
	{
		...
		word = word.ToLower();
		if (Enumerable.Contains(AllowList, word))
		{
			return false;
		}
		return Enumerable.Contains(ProfanityList, word);
	}
	...
	public bool ContainsProfanity(string text, ProfanityChechkerType checkType)
	{
		...
		case ProfanityChechkerType.FalsePositive:
			...
			{
				if (!Enumerable.Contains(AllowList, item))
				{
					return true;
				}
			}
		...
		case ProfanityChechkerType.FalseNegative:
			foreach (object item2 in new Regex("\\w(?<!\\d)[\\w'-]*", RegexOptions.IgnoreCase).Matches(text))
			{
				...
				if (Enumerable.Contains(ProfanityList, value) && !Enumerable.Contains(AllowList, value))
				{
					return true;
				}
			}
	...
	public string CensorText(string text)
	{
		if (!string.IsNullOrEmpty(text))
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				{
					string text5 = text3;
					while (text3.Contains(text4) && !Enumerable.Contains(AllowList, text3))
					{
