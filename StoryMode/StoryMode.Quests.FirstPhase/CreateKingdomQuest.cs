...
public class CreateKingdomQuest : StoryModeQuestBase
{
	...
	private TextObject _leftKingdomAfterCreatingLogText
	{
		get
		{
			//IL_0006: Unknown result type (might be due to invalid IL or missing references)
			//IL_000c: Expected O, but got Unknown
			return new TextObject("{=nNavD2NO}You left the kingdom you have created. You can only support kingdoms that you are a part of.", (Dictionary<string, object>)null);
		}
	}
	...
	private TextObject _clanIndependenceRequirementLogText
	{
		get
		{
			//IL_0006: Unknown result type (might be due to invalid IL or missing references)
			//IL_000c: Expected O, but got Unknown
			return new TextObject("{=a0ZKBj6P}Be an independent clan", (Dictionary<string, object>)null);
		}
	}
	...
	private TextObject _questFailedLogText
	{
		get
		{
			//IL_0006: Unknown result type (might be due to invalid IL or missing references)
			//IL_000c: Expected O, but got Unknown
			return new TextObject("{=tVlZTOst}You have chosen a different path.", (Dictionary<string, object>)null);
		}
	}
	...
	public CreateKingdomQuest(Hero questGiver)
	{
		...
		base._002Ector("main_storyline_create_kingdom_quest_" + ((StoryModeHeroes.ImperialMentor == questGiver) ? "1" : "0"), questGiver, StoryModeManager.Current.MainStoryLine.FirstPhase.FirstPhaseEndTime);
