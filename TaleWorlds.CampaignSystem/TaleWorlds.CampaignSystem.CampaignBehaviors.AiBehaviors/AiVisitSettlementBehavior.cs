...
{
	private readonly struct SettlementNavigationData(float distance, int settlementIdentifier, Settlement settlement, MobileParty.NavigationType bestNavigationType, bool isFromPort, bool isTargetingPortBetter) : IComparable<SettlementNavigationData>
	{
		public readonly float Distance = distance;

		public readonly int SettlementIdentifier = settlementIdentifier;

		public readonly Settlement Settlement = settlement;

		public readonly MobileParty.NavigationType BestNavigationType = bestNavigationType;

		public readonly bool IsFromPort = isFromPort;

		public readonly bool IsTargetingPortBetter = isTargetingPortBetter;
		...
		foreach (SettlementNavigationData settlementsNavigationDatum in _settlementsNavigationData)
		{
			...
			if (!mobileParty.IsDisbanding)
			{
				...
				if (disbandPartyCampaignBehavior == null || !disbandPartyCampaignBehavior.IsPartyWaitingForDisband(mobileParty))
				{
					if (leaderHero == null)
					{
						float visitingNearbySettlementScore = CalculateMergeScoreForLeaderlessParty(mobileParty, settlement, distance, out var canMerge);
						if (canMerge)
						{
