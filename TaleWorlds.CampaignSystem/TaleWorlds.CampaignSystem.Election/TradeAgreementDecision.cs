...
public class TradeAgreementDecision : KingdomDecision
{
	...
	public static Clan GetProposerClanForPlayerKingdom(Kingdom target)
	{
		...
		Clan clan = ((Clan.PlayerClan.Kingdom.RulingClan != Clan.PlayerClan && Campaign.Current.Models.TradeAgreementModel.GetScoreOfStartingTradeAgreement(Clan.PlayerClan.Kingdom, target, Clan.PlayerClan.Kingdom.RulingClan, out var _) > 50f) ? Clan.PlayerClan.Kingdom.RulingClan : null);
		if (clan == null)
		{
			List<Clan> list = Clan.PlayerClan.Kingdom.Clans.Where((Clan x) => !x.IsUnderMercenaryService && x != Clan.PlayerClan && Campaign.Current.Models.TradeAgreementModel.GetScoreOfStartingTradeAgreement(Clan.PlayerClan.Kingdom, target, x, out var _) > 50f).ToList();
