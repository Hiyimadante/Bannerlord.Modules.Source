...
public class MobilePartyAi
{
	...
	private bool GetAccessibleTargetPointInDirection(out CampaignVec2 targetPoint, Vec2 direction, float targetDistance, bool randomizeTheDirection)
	{
		...
		for (int i = 0; i < num2; i++)
		{
			...
			{
				MobileParty.NavigationType navigationType = ((!_mobileParty.IsCurrentlyAtSea) ? MobileParty.NavigationType.Default : MobileParty.NavigationType.Naval);
				if (Campaign.Current.Models.MapDistanceModel.PathExistBetweenPoints(in targetPoint, _mobileParty.Position, navigationType) && NavigationHelper.IsPositionValidForNavigationType(targetPoint.Face, navigationType) && CheckIfThereIsAnyHugeObstacleBetweenPartyAndTarget(_mobileParty, targetPoint.ToVec2()))
				{
					return true;
				}
			}
