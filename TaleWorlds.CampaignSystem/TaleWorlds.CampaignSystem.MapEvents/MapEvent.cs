...
public sealed class MapEvent : MBObjectBase
{
	...
	{
		Vec2 eventDirection = GetEventDirection(side, out var eventDistance);
	...
	private bool SimulateShipHit(MapEventSide strikerSide, MapEventSide struckSide, Ship strikerShip, SiegeEngineType siegeEngine, float advantage, float strikerSideMorale, float struckSideMorale)
	{
		...
		if (flag)
		{
			...
			int damage = (int)Campaign.Current.Models.CombatSimulationModel.SimulateHit(strikerShip, randomSimulationShip, owner, owner2, siegeEngine, advantage, this, out var troopCasualties).ResultNumber;
