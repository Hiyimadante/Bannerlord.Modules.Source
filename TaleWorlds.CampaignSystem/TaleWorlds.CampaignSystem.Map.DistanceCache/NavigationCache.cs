...
public abstract class NavigationCache<T> where T : ISettlementDataHolder
{
	...
	private void AddClosestEntrancePairBase(T settlement1, bool isPort1, T settlement2, bool isPort2)
	{
		...
		float realDistanceAndLandRatioBetweenSettlements = GetRealDistanceAndLandRatioBetweenSettlements(settlement3, settlement4, out var landRatio);
		float realDistanceAndLandRatioBetweenSettlements2 = GetRealDistanceAndLandRatioBetweenSettlements(settlement4, settlement3, out var landRatio2);
	...
	{
		bool flag = CheckBeingNeighbor(settlementsToConsider, settlement1, settlement2, useGate1, useGate2, out var foundDistance);
	...
	protected T GetClosestSettlementToPosition(Vec2 checkPosition, PathFaceRecord currentFaceRecord, int[] excludedFaceIds, List<T> settlementRecords, int regionSwitchCostTo0, int regionSwitchCostTo1, float minPathScoreEverFound, out bool isPort, bool useEarlyOut = false)
	{
		...
		foreach (T item in GetClosestSettlementsToPositionInCache(checkPosition, settlementRecords))
		{
			...
			float realPathDistanceFromPositionToSettlement = GetRealPathDistanceFromPositionToSettlement(checkPosition, currentFaceRecord, minPathScoreEverFound * 2f, item, out var isPort2);
