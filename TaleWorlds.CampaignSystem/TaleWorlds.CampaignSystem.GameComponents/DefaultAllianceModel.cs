...
public class DefaultAllianceModel : AllianceModel
{
	...
	public override float GetScoreOfCallingToWar(Kingdom callingKingdom, Kingdom calledKingdom, Kingdom kingdomToCallToWarAgainst, IFaction evaluatingFaction, out TextObject reason)
	{
		...
		float scoreOfDeclaringWar = Campaign.Current.Models.DiplomacyModel.GetScoreOfDeclaringWar(callingKingdom, kingdomToCallToWarAgainst, evaluatingClan, out var _);
	...
	{
		Clan clan = ((proposerKingdom.RulingClan != Clan.PlayerClan && Campaign.Current.Models.AllianceModel.GetSupportScoreOfStartingAllianceForClan(proposerKingdom, proposedKingdom, proposerKingdom.RulingClan, out var explanation) > 0f) ? proposerKingdom.RulingClan : null);
		if (clan == null)
		{
	...
	private (Kingdom threateningKingdom, float threatScore) GetThreateningNeighbor(Kingdom querierKingdom, out float exposureScore, out float powerRatio)
	{
		...
		{
			float num3 = CalculateThreatScore(item2.Value, num, CalculateKingdomStrength(item2.Key), CalculateKingdomStrength(querierKingdom), out var exposureScore2, out var powerRatio2);
			if (num2 < num3)
			{
				item = item2.Key;
				num2 = num3;
	...
	{
		float scoreOfDeclaringWar = Campaign.Current.Models.DiplomacyModel.GetScoreOfDeclaringWar(calledKingdom, kingdomToCallToWarAgainst, calledKingdom.RulingClan, out var _);
