...
public class DefaultMapWeatherModel : MapWeatherModel
{
	...
	private WeatherEvent SetIsBlizzardOrSnowFromFunction(float snowValue, CampaignTime campaignTime, in Vec2 position)
	{
		...
		Vec2 adjustedPosition = GetNodePositionForWeather(position, out var xIndex, out var yIndex);
	...
	{
		Vec2 nodePositionForWeather = GetNodePositionForWeather(position, out var _, out var _);
