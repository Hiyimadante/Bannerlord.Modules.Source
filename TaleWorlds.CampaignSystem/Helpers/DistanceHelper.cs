...
public static class DistanceHelper
{
	...
	private static float FindClosestDistanceFromSettlementToSettlementForMobileParty(MobileParty mobileParty, Settlement toSettlement, MobileParty.NavigationType navCapabilities, out bool isFromPort, out bool isTargetingPort, out float landRatio)
	{
		...
		if (flag2 & flag)
		{
			float num3 = Campaign.Current.Models.MapDistanceModel.GetDistance(currentSettlement, toSettlement, isFromPort: true, isTargetingPort: false, navCapabilities, out var landRatio2);
			if (flag4)
			{
		...
		if (flag3 & flag)
		{
			float distance = Campaign.Current.Models.MapDistanceModel.GetDistance(currentSettlement, toSettlement, isFromPort: false, isTargetingPort: true, navCapabilities, out var landRatio3);
			if (distance < num2)
			{
				isFromPort = false;
				isTargetingPort = true;
		...
		if (flag2 & flag3)
		{
			float num4 = Campaign.Current.Models.MapDistanceModel.GetDistance(currentSettlement, toSettlement, isFromPort: true, isTargetingPort: true, navCapabilities, out var landRatio4);
			if (flag4)
			{
	...
	public static float FindClosestDistanceFromMobilePartyToSettlement(MobileParty fromMobileParty, Settlement toSettlement, MobileParty.NavigationType navCapabilities, out bool isTargetingPort, out float landRatio)
	{
		...
		else
		{
			...
			{
				float distance = Campaign.Current.Models.MapDistanceModel.GetDistance(fromMobileParty, toSettlement, isTargetingPort: true, navCapabilities, out var estimatedLandRatio);
				if (distance < num)
				{
					isTargetingPort = true;
					num = distance;
