...
public class HeroSpawnCampaignBehavior : CampaignBehaviorBase
{
	...
	private void GiveInitialItemsToParty(MobileParty heroParty)
	{
		...
		foreach (Settlement settlement in Campaign.Current.Settlements)
		{
			...
			float distance = Campaign.Current.Models.MapDistanceModel.GetDistance(heroParty, settlement, isTargetingPort: false, heroParty.NavigationCapability, out var _);
			if (!(distance < num))
			{
				continue;
			}
