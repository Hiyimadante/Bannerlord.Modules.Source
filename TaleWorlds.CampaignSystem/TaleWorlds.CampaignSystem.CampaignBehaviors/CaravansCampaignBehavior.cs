...
public class CaravansCampaignBehavior : CampaignBehaviorBase
{
	...
	private struct PriceIndexData(float averageBuySellPriceIndex, float minBuySellPriceIndex)
	{
		internal readonly float AverageBuySellPriceIndex = averageBuySellPriceIndex;

		internal readonly float MinBuySellPriceIndex = minBuySellPriceIndex;
	}
	...
	private void UpdateAverageValues()
	{
		...
		for (int i = 0; i < ItemCategories.All.Count; i++)
		{
			...
			bool flag = dictionary.TryGetValue(itemCategory, out var value2);
	...
	public void HourlyTickParty(MobileParty mobileParty)
	{
		...
		if (flag)
		{
			...
			Town town2 = ThinkNextDestination(mobileParty, out var bestNavigationType, out var isFromPort, out var isTargetingPort);
			if (town2 != null)
			{
	...
	private Town FindNextDestinationForCaravan(MobileParty caravanParty, bool distanceCut, out MobileParty.NavigationType bestNavigationType, out bool isFromPort, out bool isTargetingPort)
	{
		...
		foreach (Town allTown in Town.AllTowns)
		{
			...
			{
				float tradeScoreForTown = GetTradeScoreForTown(caravanParty, allTown, value, input, distanceCut, out var bestNavigationType2, out var isTargetingPort2);
