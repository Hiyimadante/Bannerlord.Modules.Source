...
public class IncidentEffect
{
	...
	public static IncidentEffect SkillChange(SkillObject skill, float amount)
	{
		return new IncidentEffect(() => true, delegate
		{
			...
			textObject.SetTextVariable("AMOUNT", TaleWorlds.Library.MathF.Round(num));
	...
	{
		return new IncidentEffect(() => (from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
			where predicate != null && predicate(x) && x.Character != CharacterObject.PlayerCharacter && FindTroopToDemoteTo(x.Character, demotionPredicate, out var _)
			select x).Sum((TroopRosterElement x) => x.Number) > amount, delegate
		{
			int num = amount;
			while (num > 0)
			{
				TroopRosterElement randomElementWithPredicate = MobileParty.MainParty.MemberRoster.GetTroopRoster().GetRandomElementWithPredicate((TroopRosterElement x) => predicate(x) && x.Character != CharacterObject.PlayerCharacter && FindTroopToDemoteTo(x.Character, demotionPredicate, out var _));
	...
	private static bool FindTroopToDemoteTo(CharacterObject troop, Func<CharacterObject, bool> demotionPredicate, out CharacterObject troopToDemoteTo)
	{
		...
		if (source.Any())
		{
			...
			if (grouping != null)
			{
				CharacterObject characterObject = grouping.FirstOrDefault((CharacterObject x) => Enumerable.Contains(x.UpgradeTargets, troop));
				if (characterObject != null)
				{
		...
		if (source2.Any())
		{
			...
			if (grouping2 != null)
			{
				CharacterObject characterObject2 = grouping2.FirstOrDefault((CharacterObject x) => Enumerable.Contains(x.UpgradeTargets, troop));
