...
public sealed class Module : DotNetObject, IGameStateManagerOwner
{
	...
	private AssemblyLoader.AssemblyLoadResult AddSubModule(SubModuleInfo subModuleInfo, Assembly subModuleAssembly)
	{
		...
		AssemblyLoader.AssemblyLoadResult assemblyLoadResult = CollectModuleAssemblyTypes(subModuleInfo, subModuleAssembly, out var types);
	...
	private void LoadPlatformServices()
	{
		...
		if (assembly != null)
		{
			...
			{
				if (Enumerable.Contains<Type>(item.GetInterfaces(), typeof(IPlatformServices)))
				{
					type = item;
					break;
				}
		...
		foreach (ModuleInfo module2 in modules)
		{
			foreach (SubModuleInfo subModule in module2.SubModules)
			{
				...
				if (!string.IsNullOrEmpty(text5))
				{
					...
					if (result2 != AssemblyLoader.AssemblyLoadResult.Success)
					{
	...
	{
		XmlDocument mergedXmlForNative = MBObjectManager.GetMergedXmlForNative("soln_skins", out var usedPaths);
	...
	{
		XmlDocument mergedXmlForNative = MBObjectManager.GetMergedXmlForNative("soln_item_holsters", out var usedPaths);
		System.IO.StringWriter stringWriter = new System.IO.StringWriter();
		XmlTextWriter w = new XmlTextWriter(stringWriter);
		mergedXmlForNative.WriteTo(w);
	...
	{
		XmlDocument mergedXmlForNative = MBObjectManager.GetMergedXmlForNative("soln_action_types", out var _);
		System.IO.StringWriter stringWriter = new System.IO.StringWriter();
		XmlTextWriter w = new XmlTextWriter(stringWriter);
		mergedXmlForNative.WriteTo(w);
		return stringWriter.ToString();
	...
	{
		XmlDocument mergedXmlForNative = MBObjectManager.GetMergedXmlForNative("soln_animations", out var usedPaths);
		System.IO.StringWriter stringWriter = new System.IO.StringWriter();
		XmlTextWriter w = new XmlTextWriter(stringWriter);
		mergedXmlForNative.WriteTo(w);
		animationsXmlPaths = "";
	...
	{
		XmlDocument mergedXmlForNative = MBObjectManager.GetMergedXmlForNative("soln_sound_event_data", out var _);
		System.IO.StringWriter stringWriter = new System.IO.StringWriter();
		XmlTextWriter w = new XmlTextWriter(stringWriter);
		mergedXmlForNative.WriteTo(w);
		return stringWriter.ToString();
	...
	{
		XmlDocument mergedXmlForNative = MBObjectManager.GetMergedXmlForNative("soln_sound_parameter_data", out var _);
		System.IO.StringWriter stringWriter = new System.IO.StringWriter();
		XmlTextWriter w = new XmlTextWriter(stringWriter);
		mergedXmlForNative.WriteTo(w);
		return stringWriter.ToString();
	...
	{
		XmlDocument xmlDocument = MBObjectManager.GetMergedXmlForNative("soln_" + xmlType, out var _);
