...
public sealed class Mission : DotNetObject, IMission
{
	public class MBBoundaryCollection : IDictionary<string, ICollection<Vec2>>, ICollection<KeyValuePair<string, ICollection<Vec2>>>, IEnumerable<KeyValuePair<string, ICollection<Vec2>>>, IEnumerable, INotifyCollectionChanged
	{
		...
		public bool Remove(string name)
		{
			...
			if (flag)
			{
				CollectionChanged?.Invoke(this, new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Remove, name));
			}
			foreach (Team team in Current.Teams)
			{
				foreach (Formation item in team.FormationsIncludingSpecialAndEmpty)
	...
	public sealed class TeamCollection : List<Team>
	{
		...
		private void SetPlayerTeamAux(int index)
		{
			...
			AdjustPlayerTeams();
			OnPlayerTeamChanged?.Invoke(playerTeam, _playerTeam);
		}
	...
	public Agent MainAgent
	{
		...
		set
		{
			...
			_mainAgent = value;
			OnMainAgentChanged?.Invoke(mainAgent);
	...
	public void ResetMission()
	{
		...
		OnMissionReset?.Invoke(this, null);
	}
	...
	{
		int result = MBAPI.IMBMission.AddMissileSingleUsage(Pointer, isPrediction, shooterAgent.Index, in weaponData, in weaponStatsData, damageBonus, ref position, ref direction, ref orientation, baseSpeed, speed, addRigidBody, gameEntityToIgnore.Pointer, forcedMissileIndex, isPrimaryWeaponShot, out var missileEntity2);
		missileEntity = (isPrediction ? null : new GameEntity(missileEntity2));
		return result;
	}
	...
	{
		if (GetAgentTroopClass_Override != null)
		{
			return GetAgentTroopClass_Override(battleSide, agentCharacter);
		}
	...
	{
		if (GetOverriddenFleePositionForAgent != null)
		{
			WorldPosition? worldPosition = GetOverriddenFleePositionForAgent(agent);
	...
	{
		OnBeforeAgentRemoved?.Invoke(affectedAgent, affectorAgent, agentState, killingBlow);
	...
	public void SpawnWeaponAsDropFromAgentAux(Agent agent, EquipmentIndex equipmentIndex, ref Vec3 globalVelocity, ref Vec3 globalAngularVelocity, WeaponSpawnFlags spawnFlags, int forcedSpawnIndex)
	{
		...
		OnItemDrop?.Invoke(agent, firstScriptOfType);
	}
	...
	public void OnEndMissionRequest()
	{
		foreach (MissionLogic missionLogic in MissionLogics)
		{
			InquiryData inquiryData = missionLogic.OnEndMissionRequest(out var canLeave);
			if (!canLeave)
			{
				_leaveMissionTimer = null;
				return;
	...
	public Agent SpawnAgent(AgentBuildData agentBuildData, bool spawnFromAgentVisuals = false)
	{
		...
		if (!agentBuildData.BodyPropertiesOverriden)
		{
			...
			if (OnComputeTroopBodyProperties != null)
			{
				bodyProperties = OnComputeTroopBodyProperties(agentBuildData, agentCharacter, equipment, agentBuildData.AgentEquipmentSeed);
	...
	{
		if (IsAgentInteractionAllowed_AdditionalCondition != null)
		{
			Delegate[] invocationList = IsAgentInteractionAllowed_AdditionalCondition.GetInvocationList();
	...
	{
		if (AreOrderGesturesEnabled_AdditionalCondition != null)
		{
			Delegate[] invocationList = AreOrderGesturesEnabled_AdditionalCondition.GetInvocationList();
			for (int i = 0; i < invocationList.Length; i++)
			{
				object obj;
				if ((obj = invocationList[i].DynamicInvoke()) is bool && !(bool)obj)
	...
	{
		float agentStateProbability = MissionGameModels.Current.AgentDecideKilledOrUnconsciousModel.GetAgentStateProbability(affectorAgent, agent, damageType, weaponFlags, out var useSurgeryProbability);
	...
	{
		OnAddSoundAlarmFactorToAgents?.Invoke(alarmCreatorAgent, in soundPosition, soundLevelSquareRoot);
	}
	...
	public bool CanAgentRout(Agent agent)
	{
		...
		{
			if (CanAgentRout_AdditionalCondition != null)
			{
				return CanAgentRout_AdditionalCondition(agent);
			}
			return true;
		}
		return false;
	...
	internal void MeleeHitCallback(ref AttackCollisionData collisionData, Agent attacker, Agent victim, GameEntity realHitEntity, ref float inOutMomentumRemaining, ref MeleeCollisionReaction colReaction, CrushThroughState crushThroughState, Vec3 blowDir, Vec3 swingDir, ref HitParticleResultData hitParticleResultData, bool crushedThroughWithoutAgentCollision)
	{
		...
		{
			AddSoundAlarmFactorToAgents(attacker, collisionData.CollisionGlobalPosition, 15f);
		}
		...
		{
			AddSoundAlarmFactorToAgents(attacker, collisionData.CollisionGlobalPosition, 8f);
		}
	}
	...
	internal void MissileAreaDamageCallback(ref AttackCollisionData collisionDataInput, ref Blow blowInput, Agent alreadyDamagedAgent, Agent shooterAgent, bool isBigExplosion)
	{
		...
		OnCameraShakeTriggered?.Invoke(in blowInput.GlobalPosition, 10f + num * 5f);
	}
	...
	internal void OnMissileRemoved(int missileIndex)
	{
		...
		OnMissileRemovedEvent?.Invoke(missileIndex);
	}
	...
	public bool IsFormationUnitPositionAvailableMT(ref WorldPosition formationPosition, ref WorldPosition unitPosition, ref WorldPosition nearestAvailableUnitPosition, float manhattanDistance, Team team)
	{
		...
		if (IsFormationUnitPositionAvailable_AdditionalCondition != null && !IsFormationUnitPositionAvailable_AdditionalCondition(unitPosition, team))
		{
			return false;
		}
	...
	public bool IsOrderPositionAvailable(in WorldPosition orderPosition, Team team)
	{
		...
		if (IsFormationUnitPositionAvailable_AdditionalCondition != null && !IsFormationUnitPositionAvailable_AdditionalCondition(orderPosition, team))
		{
			return false;
		}
	...
	public void OnDeploymentFinished()
	{
		...
		DeploymentFinishedEvent?.Invoke();
	}
	...
	{
		FormationCaptainChanged?.Invoke(formation);
	}
	...
	{
		return IsBattleInRetreatEvent?.Invoke() ?? false;
	}
	...
	{
		OnItemPickUp?.Invoke(agent, spawnedItemEntity);
	}
