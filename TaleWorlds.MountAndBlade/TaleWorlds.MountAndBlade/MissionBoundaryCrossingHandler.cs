...
public class MissionBoundaryCrossingHandler : MissionLogic
{
	...
	private void OnAgentWentOut(Agent agent, float startTimeInSeconds)
	{
		...
		if (base.Mission.MainAgent == agent)
		{
			...
			StartTime?.Invoke(_leewayTime, 0f);
	...
	public override void OnMissionTick(float dt)
	{
		...
		if (_mainAgentLeaveTimer != null)
		{
			...
			TimeCount?.Invoke(obj);
		}
	}

	private void TickForMainAgent()
	{
		bool isAgentOutside = ((_vehicleHandler == null || !_vehicleHandler.IsAgentInVehicle(Agent.Main, out var vehicleEntity)) ? (!base.Mission.IsPositionInsideBoundaries(Agent.Main.Position.AsVec2)) : (!base.Mission.IsPositionInsideBoundaries(vehicleEntity.GlobalPosition.AsVec2)));
