...
public class LadderQueueManager : MissionObject
{
	...
	private float GetScoreForAddingAgentToQueueIndex(Vec3 agentPosition, int queueIndex, out int scoreOfQueueIndex)
	{
		...
		if (_queuedAgents.Count <= queueIndex || _queuedAgents[queueIndex] == null)
		{
			...
			{
				float scoreForAddingAgentToQueueIndex = GetScoreForAddingAgentToQueueIndex(agentPosition, parentIndex, out var scoreOfQueueIndex2);
			...
			{
				float scoreForAddingAgentToQueueIndex2 = GetScoreForAddingAgentToQueueIndex(agentPosition, parentIndex2, out var scoreOfQueueIndex3);
	...
	private void AddAgentToQueue(Agent agent)
	{
		...
		{
			float scoreForAddingAgentToQueueIndex = GetScoreForAddingAgentToQueueIndex(position, GetQueueIndexForCoordinates(new Vec2i(i, y)), out var scoreOfQueueIndex);
