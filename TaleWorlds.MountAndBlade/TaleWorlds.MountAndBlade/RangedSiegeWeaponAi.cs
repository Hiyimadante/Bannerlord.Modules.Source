...
public abstract class RangedSiegeWeaponAi : UsableMachineAIBase
{
	public class ThreatSeeker
	{
		...
		{
			float minimumDistanceBetweenPositions = GetMinimumDistanceBetweenPositions(formation.GetMedianAgent(excludeDetachedUnits: false, excludePlayer: false, formation.GetAveragePositionOfUnits(excludeDetachedUnits: false, excludePlayer: false)).Position, referencePositions, out var closestCastlePosition);
