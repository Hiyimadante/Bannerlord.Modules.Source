...
public class MissionScoreboardComponent : MissionNetwork
{
	...
	{
		private readonly Func<MissionPeer, string> _playerGetterFunc = playerGetterFunc;

		private readonly Func<BotData, string> _botGetterFunc = botGetterFunc;

		public readonly string Id = id;

		public readonly TextObject Name = GameTexts.FindText("str_scoreboard_header", id);
	...
	public void ChangeTeamScore(Team team, int scoreChange)
	{
		...
		if (OnRoundPropertiesChanged != null)
		{
	...
	public void HandleServerSetRoundMVP(GameNetworkMessage baseMessage)
	{
		...
		OnMVPSelected?.Invoke(setRoundMVP.MVPPeer.GetComponent<MissionPeer>(), setRoundMVP.MVPCount);
	...
	private void TeamChange(NetworkCommunicator player, Team oldTeam, Team nextTeam)
	{
		...
		{
			OnPlayerSideChanged(oldTeam, nextTeam, component);
		}
	}
	...
	public override void OnPlayerDisconnectedFromServer(NetworkCommunicator networkPeer)
	{
		...
		else
		{
			...
			OnPlayerSideChanged?.Invoke(missionPeer.Team, null, missionPeer);
		}
	}
	...
	{
		if (OnBotPropertiesChanged != null)
		{
			OnBotPropertiesChanged(side);
		}
	}
	...
	public void PlayerPropertiesChanged(MissionPeer player)
	{
		if (!GameNetwork.IsDedicatedServer)
		{
			...
			if (OnPlayerPropertiesChanged != null && player.Team != null && player.Team != Mission.Current.SpectatorTeam)
			{
				BattleSideEnum side = player.Team.Side;
				OnPlayerPropertiesChanged(side, player);
			}
		}
	}
	...
	private void LateInitializeHeaders()
	{
		if (!_isInitialized)
		{
			...
			if (OnScoreboardInitialized != null)
			{
	...
	private void SetPeerAsMVP(MissionPeer peer)
	{
		...
		GameNetwork.EndBroadcastModuleEvent(GameNetwork.EventBroadcastFlags.None);
		OnMVPSelected?.Invoke(peer, num2);
	}
