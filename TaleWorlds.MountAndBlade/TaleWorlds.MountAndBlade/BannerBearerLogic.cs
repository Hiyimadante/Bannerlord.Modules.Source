...
public class BannerBearerLogic : MissionLogic
{
	private class FormationBannerController
	{
		...
		public bool HasBannerEntity(GameEntity bannerEntity)
		{
			...
			{
				return Enumerable.Contains(_bannerInstances.Keys, bannerEntity.Pointer);
			}
			return false;
		}
		...
		{
			return Enumerable.Contains(_bannerSearchers.Keys, agent);
		}
		...
		public void UpdateBannerSearchers()
		{
			...
			foreach (GameEntity banner in bannersOnGround)
			{
				...
				if (flag)
				{
					Agent agent = FindBestSearcherForBanner(banner, out var distance);
					if (agent != null)
					{
	...
	public void UpdateAgent(Agent agent, bool willBecomeBannerBearer)
	{
		...
		OnBannerBearerAgentUpdated?.Invoke(agent, willBecomeBannerBearer);
	}
