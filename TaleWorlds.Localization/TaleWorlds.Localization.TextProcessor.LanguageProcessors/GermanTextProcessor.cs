...
public class GermanTextProcessor : LanguageSpecificTextProcessor
{
	...
	private struct DictionaryWord(string nominative, string nominativePlural, string genitive, string genitivePlural, string dative, string dativePlural, string accusative, string accusativePlural)
	{
	...
	private void HandlePossession(StringBuilder outputString, int cursorPos)
	{
		...
		if (Enumerable.Contains(SSounds, char.ToLower(lastCharacter, CultureInfo)))
		{
			outputString.Append('\'');
		}
		else
