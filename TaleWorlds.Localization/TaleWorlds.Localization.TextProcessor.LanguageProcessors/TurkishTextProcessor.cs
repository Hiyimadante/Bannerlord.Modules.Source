...
public class TurkishTextProcessor : LanguageSpecificTextProcessor
{
	...
	{
		return Enumerable.Contains(Vowels, char.ToLower(c, CultureInfoForLanguage));
	}
	...
	private char GetNextVowel(StringBuilder stringBuilder)
	{
		...
		if (!Enumerable.Contains(BackVowels, char.ToLower(value, CultureInfoForLanguage)))
		{
			return 'e';
		}
		return 'a';
	...
	private char GetNextVowel(int number)
	{
		...
		if (num != 0)
		{
			if (!Enumerable.Contains(BackNumbers, num))
			{
				return 'e';
			}
			return 'a';
		...
		if (num2 != 0)
		{
			if (!Enumerable.Contains(BackNumbers, num2))
			{
				return 'e';
			}
			return 'a';
	...
	{
		return Enumerable.Contains(FrontVowels, char.ToLower(c, CultureInfoForLanguage));
	}

	private bool IsClosedVowel(char c)
	{
		return Enumerable.Contains(ClosedVowels, char.ToLower(c, CultureInfoForLanguage));
	}

	private bool IsConsonant(char c)
	{
		return Enumerable.Contains(Consonants, char.ToLower(c, CultureInfoForLanguage));
	}

	private bool IsUnvoicedConsonant(char c)
	{
		return Enumerable.Contains(UnvoicedConsonants, char.ToLower(c, CultureInfoForLanguage));
	}

	private bool IsHardUnvoicedConsonant(char c)
	{
		return Enumerable.Contains(HardUnvoicedConsonants, char.ToLower(c, CultureInfoForLanguage));
	}
	...
	private bool LastWordNonMutating(StringBuilder outputString)
	{
		...
		if (lastWord != null)
		{
			return Enumerable.Contains(NonMutatingWord, lastWord.ToLower(CultureInfoForLanguage));
		}
		return false;
	}
