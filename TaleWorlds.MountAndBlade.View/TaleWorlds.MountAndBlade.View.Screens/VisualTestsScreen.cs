...
public class VisualTestsScreen : ScreenBase
{
	...
	public VisualTestsScreen(bool isValidTest, ConfigQuality preset, string sceneName, DateTime testTime, List<string> testTypesToCheck)
	{
		...
		_validWriteDirectory = Utilities.GetVisualTestsValidatePath();
		_validReadDirectory = Utilities.GetBasePath() + "ValidVisuals/";
		_pathDirectory = Utilities.GetVisualTestsTestFilesPath();
		_failDirectory = TestCommonBase.GetAttachmentsFolderPath();
		_reportFile = "report.txt";
		isValidTest_ = true;
		frameCounter = -200;
		testTypesToCheck_ = new List<string>();
