...
public class NavalMobilePartyVisual : MapEntityVisual<PartyBase>
{
	...
	public NavalMobilePartyVisual(PartyBase partyBase)
	{
		...
		_shipToBlockadeShipVisualCache = new Dictionary<Ship, BlockadeShipVisual>();
		_oars = new List<ShipOar>();
		_sailVisualCache = new List<SailVisual>();
		_splashFoamDecals = new ShipFoamDecal[20];
		_lastDecalSpawnPosition = Vec3.Zero;
		_nextDecalSpawnMetersSq = 0.09f;
		base._002Ector(partyBase);
	...
	internal void Tick(float dt, float realDt, ref int dirtyPartiesCount, ref NavalMobilePartyVisual[] dirtyPartiesList)
	{
		...
		if (_raidAgentVisuals != null)
		{
			...
			_raidAgentVisuals.Tick((AgentVisuals)null, dt, false, num2);
		}
	}
	...
	internal void UpdateEntityPosition(float dt, float realDt, bool isVisible = false)
	{
		...
		else if (mobileParty.CurrentSettlement == null)
		{
			...
			Vec3 val7 = val5 + val6;
			float num6 = Vec3.AngleBetweenTwoVectors(entityFrame.rotation.u, val7) * 0.5f;
			...
			else if (base.MapEntity.MobileParty != null && (base.MapEntity.MobileParty.IsCurrentlyAtSea || base.MapEntity.MobileParty.IsTransitionInProgress) && (base.MapEntity.MobileParty.CurrentSettlement == null || base.MapEntity.MobileParty.IsTargetingPort))
			{
				...
				if (base.MapEntity.MobileParty.MapEvent != null)
				{
					...
					if (mapEventSettlement != null && mapEventSettlement.IsVillage)
					{
						...
						CampaignVec2 position = base.MapEntity.MobileParty.MapEvent.MapEventSettlement.Position;
						identity.origin = ((CampaignVec2)(ref position)).AsVec3();
						((Mat3)(ref identity.rotation)).ApplyScaleLocal(_raidAgentVisuals.GetScale());
						WeakGameEntity weakEntity = _raidAgentVisuals.GetWeakEntity();
						((WeakGameEntity)(ref weakEntity)).SetFrame(ref identity, true);
					}
				}
			}
		}
	...
	private void AddRaidPartyVisual(PartyBase party)
	{
		...
		Equipment val = ((BasicCharacterObject)visualPartyLeader).Equipment.Clone(false);
		GetMeleeWeaponToWield(party, out var wieldedItemIndex);
		Monster baseMonsterFromRace = FaceGen.GetBaseMonsterFromRace(((BasicCharacterObject)visualPartyLeader).Race);
		MBActionSet actionSetWithSuffix = MBGlobals.GetActionSetWithSuffix(baseMonsterFromRace, ((BasicCharacterObject)visualPartyLeader).IsFemale, "_map_with_banner");
		AgentVisualsData obj = new AgentVisualsData().UseMorphAnims(true).Equipment(val).BodyProperties(((BasicCharacterObject)visualPartyLeader).GetBodyProperties(((BasicCharacterObject)visualPartyLeader).Equipment, -1))
			.SkeletonType((SkeletonType)(((BasicCharacterObject)visualPartyLeader).IsFemale ? 1 : 0))
			.Scale(0.3f)
		...
		AgentVisualsData obj2 = obj.ClothColor1((mapFaction != null) ? mapFaction.Color : 4291609515u);
		IFaction mapFaction2 = party.MapFaction;
		AgentVisualsData val2 = obj2.ClothColor2((mapFaction2 != null) ? mapFaction2.Color2 : 4291609515u).CharacterObjectStringId(((MBObjectBase)visualPartyLeader).StringId).Race(((BasicCharacterObject)visualPartyLeader).Race);
		_raidAgentVisuals = AgentVisuals.Create(val2, "PartyIcon " + ((BasicCharacterObject)visualPartyLeader).Name, false, false, false);
		if (_raidAgentVisuals != null)
		{
			MBSkeletonExtensions.SetAgentActionChannel(_raidAgentVisuals.GetVisuals().GetSkeleton(), 0, ref ActionIndexCache.act_map_raid, MBRandom.NondeterministicRandomFloat * 0.7f, -0.2f, true, 0f);
			WeakGameEntity weakEntity = _raidAgentVisuals.GetWeakEntity();
			uint value = (FactionManager.IsAtWarAgainstFaction(party.MapFaction, Hero.MainHero.MapFaction) ? 4294905856u : 4278206719u);
			((WeakGameEntity)(ref weakEntity)).SetContourColor((uint?)value, false);
			float num = MathF.Min(0.25f, 20f);
			_raidAgentVisuals.Tick((AgentVisuals)null, 0.0001f, false, num);
			((WeakGameEntity)(ref weakEntity)).Skeleton.ForceUpdateBoneFrames();
		}
	}
	...
	{
		if ((((List<Ship>)(object)base.MapEntity.MobileParty.Ships).Count <= 0 && !base.MapEntity.MobileParty.IsInRaftState) || !base.MapEntity.MobileParty.IsCurrentlyAtSea || (base.MapEntity.MobileParty.CurrentSettlement != null && !base.MapEntity.MobileParty.IsTargetingPort))
		{
			if (((List<Ship>)(object)base.MapEntity.MobileParty.Ships).Count > 0 && base.MapEntity.MobileParty.SiegeEvent?.BesiegedSettlement != null)
			{
