...
public class NavalDLCViewCheats
{
	...
	public static string FocusShip(List<string> strings)
	{
		...
		if (val != null)
		{
			...
			Town val2 = ((IEnumerable<Town>)Town.AllTowns).FirstOrDefault((Town x) => ((List<Ship>)(object)x.AvailableShips).Exists((Predicate<Ship>)((Ship y) => ((MBObjectBase)y.ShipHull).StringId == shipHullStringId)));
			if (val2 != null)
			{
