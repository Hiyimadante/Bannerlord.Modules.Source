...
public class NavalMapSceneWrapper : INavalMapSceneWrapper
{
	...
	public NavalMapSceneWrapper()
	{
		...
		_pirateSpawnPoints = new Dictionary<string, List<(CampaignVec2, float)>>();
		base._002Ector();
	...
	private void InitializeDropOffLocations()
	{
		...
		{
			Village? obj = ((IEnumerable<Village>)Village.All).FirstOrDefault((Village x) => ((MBObjectBase)((SettlementComponent)x).Settlement).StringId == entity.Parent.Name);
