...
public class FaceGenVM : ViewModel
{
	...
	private void FilterCategories()
	{
		...
		IsBodyEnabled = Enumerable.Contains(availableStages, FaceGeneratorStage.Body);
		IsFaceEnabled = Enumerable.Contains(availableStages, FaceGeneratorStage.Face);
		IsEyesEnabled = Enumerable.Contains(availableStages, FaceGeneratorStage.Eyes);
		IsNoseEnabled = Enumerable.Contains(availableStages, FaceGeneratorStage.Nose);
		IsMouthEnabled = Enumerable.Contains(availableStages, FaceGeneratorStage.Mouth);
		IsHairEnabled = Enumerable.Contains(availableStages, FaceGeneratorStage.Hair);
		IsTaintEnabled = Enumerable.Contains(availableStages, FaceGeneratorStage.Taint);
	...
	private void UpdateRaceAndGenderBasedResources()
	{
		...
		{
			if (Enumerable.Contains(source2, i) || i == _faceGenerationParams.CurrentBeard)
			{
		...
		for (int j = 0; j < num; j++)
		{
			if (Enumerable.Contains(source, j) || j == _faceGenerationParams.CurrentHair)
			{
