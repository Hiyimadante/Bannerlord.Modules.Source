...
public class MapConversationTableau
{
	...
	public MapConversationTableau()
	{
		...
		_animationFrequencyThreshold = 2.5f;
	...
	private void FirstTimeInit()
	{
		...
		if (party != null)
		{
			...
			if (((memberRoster != null) ? new int?(memberRoster.TotalManCount) : ((int?)null)) > 1)
			{
				...
				IOrderedEnumerable<TroopRosterElement> orderedEnumerable = ((IEnumerable<TroopRosterElement>)_data.ConversationPartnerData.Party.MemberRoster.GetTroopRoster()).OrderByDescending(delegate(TroopRosterElement t)
				{
				...
				{
					num--;
					TroopRosterElement val2 = orderedEnumerable.FirstOrDefault(delegate(TroopRosterElement troop)
					{
						//IL_0000: Unknown result type (might be due to invalid IL or missing references)
						return !((BasicCharacterObject)troop.Character).IsHero;
					});
