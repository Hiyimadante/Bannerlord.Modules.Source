...
public class MapScreen : ScreenBase, IMapStateHandler, IGameStateListener, IChatLogHandlerScreen
{
	...
	public MapScreen(MapState mapState)
	{
		...
		CharacterBannerMaterialCache = new Dictionary<Tuple<Material, Banner>, Material>();
		MapCursor = new MapCursor();
		_timeToggleTimer = float.MaxValue;
		_activatedFrameNo = Utilities.EngineFrameNo;
		IsSoundOn = true;
		_mapSceneCursorWanted = true;
		_currentTutorialContext = (TutorialContexts)4;
		_intersectedEntityIDs = new UIntPtr[128];
		_intersectionInfos = (Intersection[])(object)new Intersection[128];
	...
	public void OnHourlyTick()
	{
		...
		_isKingdomDecisionsDirty = ((kingdom != null) ? ((IEnumerable<KingdomDecision>)kingdom.UnresolvedDecisions).FirstOrDefault((KingdomDecision d) => d.NotifyPlayer && d.IsEnforced && d.IsPlayerParticipant && !d.ShouldBeCancelled()) : null) != null;
	}
	...
	private void ShowNextKingdomDecisionPopup()
	{
		...
		Kingdom kingdom = Clan.PlayerClan.Kingdom;
		KingdomDecision val = ((kingdom != null) ? ((IEnumerable<KingdomDecision>)kingdom.UnresolvedDecisions).FirstOrDefault((KingdomDecision d) => d.NotifyPlayer && d.IsEnforced && d.IsPlayerParticipant && !d.ShouldBeCancelled()) : null);
		if (val != null)
		{
	...
	{
		return new MenuViewContext((ScreenBase)(object)this, menuContext);
	}
