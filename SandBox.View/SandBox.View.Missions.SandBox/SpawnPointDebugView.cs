...
public class SpawnPointDebugView : ScriptComponentBehavior
{
	...
	private List<List<string>> GetLevelCombinationsToCheck()
	{
		...
		gameEntity = ((ScriptComponentBehavior)this).GameEntity;
		bool flag = ((WeakGameEntity)(ref gameEntity)).Scene.GetUpgradeLevelMaskOfLevelName("naval_raid") != 0;
	...
	protected override bool OnCheckForProblems()
	{
		...
		foreach (List<string> item in GetLevelCombinationsToCheck())
		{
			...
			if (_inaccessibleEntitiesList.Any())
			{
				...
				{
					string text2 = ("There is an inaccessible position: " + ((object)inaccessibleEntities.GlobalPosition/*cast due to constrained. prefix*/).ToString()) ?? "";
					MBEditor.AddEditorWarning("Level: " + string.Join(" - ", item));
					MBEditor.AddEntityWarning(inaccessibleEntities.WeakEntity, text2);
				}
			}
			foreach (SpawnPointUnits spUnits in _spUnitsList)
			{
				if (spUnits.Place == SpawnPointUnits.SceneType.All || _sceneType == spUnits.Place)
				{
					...
					if (!flag2)
					{
						string text3 = "Spawnpoint (" + spUnits.SpName + ") has some issues. ";
						text3 = ((spUnits.MaxCount >= spUnits.CurrentCount) ? (text3 + "It is placed too less. Placed count(" + spUnits.CurrentCount + "). Min count(" + spUnits.MinCount + "). Level: " + text) : (text3 + "It is placed too much. Placed count(" + spUnits.CurrentCount + "). Max count(" + spUnits.MaxCount + "). Level: " + text));
	...
	private void AddPositionToInvalidList(CategoryId categoryId, Vec3 globalPosition, GameEntity entity, bool isDisabledNavMesh, bool doNotShowWarning = false)
	{
		...
		if (!entity.IsGhostObject() && entity.IsVisibleIncludeParents() && _invalidSpawnPointsDictionary.ContainsKey(categoryId))
		{
			...
			if (_invalidSpawnPointsDictionary[categoryId].All(delegate(InvalidPosition x)
			{
				//IL_0001: Unknown result type (might be due to invalid IL or missing references)
				//IL_0007: Unknown result type (might be due to invalid IL or missing references)
				return x.position != globalPosition;
			}))
			{
	...
	private void CheckInaccesiblePositions()
	{
		...
		if (_selectedEntity != (GameEntity)null)
		{
			Vec3 globalPosition;
			for (int j = 0; j < _allGameEntitiesWithAnimationScript.Count; j++)
			{
				...
				if (_targetPositionNavMesh.FaceGroupIndex != _disabledFaceId && _targetPositionNavMesh.FaceGroupIndex >= 0 && val2.IsVisibleIncludeParents())
				{
			...
			MBEditor.AddEditorWarning("Scene has inaccessible point!");
			{
				foreach (GameEntity inaccessibleEntities in _inaccessibleEntitiesList)
				{
					...
					for (int k = 0; k < _allGameEntitiesWithAnimationScript.Count; k++)
					{
						...
						if (_startPositionNavMesh.FaceGroupIndex != _disabledFaceId && _startPositionNavMesh.FaceGroupIndex >= 0 && val4.IsVisibleIncludeParents())
						{
	...
	public SpawnPointDebugView()
	{
		...
		_onSameLineNeeded = true;
		_redColor = new Vec3(200f, 0f, 0f, 255f);
		_greenColor = new Vec3(0f, 200f, 0f, 255f);
		_blueColor = new Vec3(0f, 180f, 180f, 255f);
		_yellowColor = new Vec3(200f, 200f, 0f, 255f);
		_purbleColor = new Vec3(255f, 0f, 255f, 255f);
		_npcDebugLineColor = 4294901760u;
		_chairDebugLineColor = 4278255360u;
		_animalDebugLineColor = 4279356620u;
		_semivalidChairDebugLineColor = 4294963200u;
		_passageDebugLineColor = 4288217241u;
		_missionBoundDebugLineColor = uint.MaxValue;
		_invalidSpawnPointsDictionary = new Dictionary<CategoryId, List<InvalidPosition>>();
		_spUnitsList = new List<SpawnPointUnits>();
		_allGameEntitiesWithAnimationScript = new List<GameEntity>();
		_inaccessibleEntitiesList = new List<GameEntity>();
		_closeEntitiesToInaccessible = new List<GameEntity>();
