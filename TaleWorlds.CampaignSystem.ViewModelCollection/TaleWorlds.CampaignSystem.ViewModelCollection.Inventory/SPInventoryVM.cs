...
public class SPInventoryVM : ViewModel
{
	...
	private class RosterElementComparer : IComparer<SPItemVM>
	{
		...
		public int Compare(SPItemVM x, SPItemVM y)
		{
			...
			{
				int num = _inventoryCapacityModel.GetItemEffectiveWeight(equipmentElement, _currentParty, _currentParty.IsCurrentlyAtSea, out var _).CompareTo(_inventoryCapacityModel.GetItemEffectiveWeight(equipmentElement2, _currentParty, _currentParty.IsCurrentlyAtSea, out var _));
				if (num != 0)
				{
					return num;
				}
	...
	private void RefreshTransactionCost(int transactionCount = 1)
	{
		...
		{
			int itemTotalPrice = _inventoryLogic.GetItemTotalPrice(_selectedItem.ItemRosterElement, transactionCount, out var lastPrice, _selectedItem.InventorySide == InventoryLogic.InventorySide.OtherInventory);
	...
	private void TransferAll(bool isBuy)
	{
		...
		else
		{
			...
			while (0 <= num4)
			{
				...
				if (!flag3)
				{
					float num6 = (flag2 ? sPItemVM2.ItemRosterElement.EquipmentElement.GetEquipmentElementWeight() : inventoryCapacityModel.GetItemEffectiveWeight(sPItemVM2.ItemRosterElement.EquipmentElement, mobileParty, mobileParty.IsCurrentlyAtSea, out var _));
