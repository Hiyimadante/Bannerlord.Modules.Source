...
public class ClanPartiesVM : ViewModel
{
	...
	{
		bool canDisbandParty = GetCanDisbandParty(out var cannotDisbandReason);
		...
		foreach (Hero item in _faction.Heroes.Where((Hero h) => !h.IsDisabled).Union(_faction.Companions))
		{
			...
			{
				bool flag = FactionHelper.IsMainClanMemberAvailableForPartyLeaderChange(item, isSend: true, CurrentSelectedParty.Party.MobileParty, out var explanation);
